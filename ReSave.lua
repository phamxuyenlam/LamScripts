...
local resaveversion = "2.26.upg2.4"
...
-- ============================================================
-- ReSave upg2: internal Log Console core
-- ============================================================
local ReSaveLogEntries = {}
local ReSaveLogMaxEntries = 500
local ReSaveLogRender = nil
local ReSaveLogSequence = 0
local ReSaveLogLastFingerprint = nil
local ReSaveLogLastClock = 0

local function ReSaveGetTimestamp()
...
local function ReSaveExtractLine(text)
    local message = tostring(text or "")
    local line = message:match(":(%d+):")
    if not line then
        line = message:match("[Ll]ine%s+(%d+)")
    end
    return tonumber(line)
end

local function ReSaveTrace(err)
...
local function ReSaveLog(level, eventName, message, details)
    local normalizedLevel = tostring(level or "INFO")
    local normalizedEvent = tostring(eventName or "APP")
    local normalizedMessage = tostring(message or "")
    local normalizedDetails = details and tostring(details) or ""

    -- Ignore an exact duplicate emitted almost immediately by overlapping
    -- engine/UI listeners. Legitimate repeated actions remain visible once
    -- they are separated by a small time interval.
    local now = os.clock()
    local fingerprint = normalizedLevel .. "\0" .. normalizedEvent .. "\0" .. normalizedMessage .. "\0" .. normalizedDetails
    if fingerprint == ReSaveLogLastFingerprint and (now - ReSaveLogLastClock) < 0.20 then
        return
    end
    ReSaveLogLastFingerprint = fingerprint
    ReSaveLogLastClock = now

    ReSaveLogSequence += 1

    local entry = {
        id = ReSaveLogSequence,
        time = ReSaveGetTimestamp(),
        level = normalizedLevel,
        event = normalizedEvent,
        message = normalizedMessage,
        details = normalizedDetails ~= "" and normalizedDetails or nil,
    }

    ReSaveLogEntries[#ReSaveLogEntries + 1] = entry
    while #ReSaveLogEntries > ReSaveLogMaxEntries do
        table.remove(ReSaveLogEntries, 1)
    end

    if ReSaveLogRender then
        task.defer(function()
            pcall(ReSaveLogRender)
        end)
    end
end

-- Capture engine/client errors when Roblox exposes these logging events.
-- Both connections are optional: if the environment blocks the service/event,
-- ReSave continues running and records that fact in the console instead.
do
        ...
        local connectOK, connectErr = pcall(function()
            ScriptContext.Error:Connect(function(message, stackTrace, scriptInstance)
                ...
                ReSaveLog(
                    ...
                    "CLIENT ERROR",
                    tostring(message),
                    "script=" .. scriptName .. "\nstack=" .. tostring(stackTrace or "<no stack>")
                )
            end)
        end)
        if not connectOK then
            ReSaveLog("WARN", "LOGGER", "ScriptContext.Error unavailable", tostring(connectErr))
        end
    else
        ReSaveLog("WARN", "LOGGER", "ScriptContext service unavailable", tostring(ScriptContext))
    end

    local logOK, LogService = pcall(function()
        return game:GetService("LogService")
    end)
    if logOK and LogService then
        local connectOK, connectErr = pcall(function()
        ...
        if not connectOK then
            ReSaveLog("WARN", "LOGGER", "LogService.MessageOut unavailable", tostring(connectErr))
        end
    else
        ReSaveLog("WARN", "LOGGER", "LogService unavailable", tostring(LogService))
    end
end

-- ============================================================
-- ReSave Part 2 utility: animated status toast
-- ============================================================
local ReSaveToastGeneration = 0
local function ReSaveNotify(message, duration)
    ReSaveLog("INFO", "NOTIFICATION", tostring(message or ""))
    local root = G2L["1"]
    if not root or not root.Parent then return end

    duration = tonumber(duration) or 1.8
    ReSaveToastGeneration += 1
    local myGeneration = ReSaveToastGeneration

    local toast = root:FindFirstChild("ReSaveToast")
    if not toast then
        toast = Instance.new("Frame")
        toast.Name = "ReSaveToast"
        toast.AnchorPoint = Vector2.new(0.5, 0)
        toast.Position = UDim2.new(0.5, 0, 0, -42)
        toast.Size = UDim2.new(0, 320, 0, 38)
        toast.BackgroundColor3 = Color3.fromRGB(20, 16, 27)
        ...
        stroke.Color = Color3.fromRGB(151, 94, 246)
        ...
        label.TextSize = 13
        label.TextColor3 = Color3.fromRGB(244, 241, 252)
    ...
    local stroke = toast:FindFirstChild("ToastStroke")
    ...
    local enterInfo = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
    local leaveInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In)

    TweenService:Create(toast, enterInfo, {
        Position = UDim2.new(0.5, 0, 0, 16),
        BackgroundTransparency = 0.08,
    }):Play()
    TweenService:Create(label, enterInfo, {TextTransparency = 0}):Play()
    if stroke then
        TweenService:Create(stroke, enterInfo, {Transparency = 0.18}):Play()
    end

    task.delay(duration, function()
        if myGeneration ~= ReSaveToastGeneration then return end
        if not toast or not toast.Parent then return end

        TweenService:Create(toast, leaveInfo, {
            Position = UDim2.new(0.5, 0, 0, -42),
            BackgroundTransparency = 1,
        }):Play()
        TweenService:Create(label, leaveInfo, {TextTransparency = 1}):Play()
        if stroke then
            TweenService:Create(stroke, leaveInfo, {Transparency = 1}):Play()
        end

        task.wait(0.23)
        if myGeneration == ReSaveToastGeneration and toast and toast.Parent then
            toast.Visible = false
        end
    end)
end
...
G2L["5d"]["CanvasSize"] = UDim2.new(0, 0, 0, 128);
...
-- StarterGui.ReSave.ScriptHubMenu.Background.ScrollingFrame.ShiftLock
G2L["shiftlock_button"] = Instance.new("TextButton", G2L["5d"]);
G2L["shiftlock_button"]["BorderSizePixel"] = 0;
G2L["shiftlock_button"]["TextXAlignment"] = Enum.TextXAlignment.Left;
G2L["shiftlock_button"]["BackgroundColor3"] = Color3.fromRGB(30, 30, 30);
G2L["shiftlock_button"]["TextSize"] = 14;
G2L["shiftlock_button"]["FontFace"] = Font.new([[rbxasset://fonts/families/Arial.json]], Enum.FontWeight.Regular, Enum.FontStyle.Normal);
G2L["shiftlock_button"]["TextColor3"] = Color3.fromRGB(255, 255, 255);
G2L["shiftlock_button"]["Size"] = UDim2.new(0, 107, 0, 18);
G2L["shiftlock_button"]["Name"] = [[ShiftLock]];
G2L["shiftlock_button"]["BorderColor3"] = Color3.fromRGB(151, 102, 255);
G2L["shiftlock_button"]["Text"] = [[ShiftLock]];
G2L["shiftlock_button"]["Position"] = UDim2.new(0.008999999612569809, 0, 0.4620000123977661, 0);
G2L["shiftlock_button"]["BackgroundTransparency"] = 1;
...
-- StarterGui.ReSave.ScriptHubMenu.ShiftLock
G2L["shiftlock_frame"] = Instance.new("Frame", G2L["5b"]);
G2L["shiftlock_frame"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
G2L["shiftlock_frame"]["BackgroundTransparency"] = 1;
G2L["shiftlock_frame"]["Size"] = UDim2.new(0, 100, 0, 100);
G2L["shiftlock_frame"]["Visible"] = false;
G2L["shiftlock_frame"]["Name"] = [[ShiftLock]];

-- StarterGui.ReSave.ScriptHubMenu.ShiftLock.Image
G2L["shiftlock_image"] = Instance.new("ImageLabel", G2L["shiftlock_frame"]);
G2L["shiftlock_image"]["BorderSizePixel"] = 0;
G2L["shiftlock_image"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
G2L["shiftlock_image"]["Image"] = [[rbxassetid://94251318161622]];
G2L["shiftlock_image"]["Size"] = UDim2.new(0, 272, 0, 126);
G2L["shiftlock_image"]["Name"] = [[Image]];
G2L["shiftlock_image"]["BackgroundTransparency"] = 1;
G2L["shiftlock_image"]["Position"] = UDim2.new(1.283868670463562, 0, 0.38265305757522583, 0);
...
function module:AddScriptTabSave(name, source)
    local tabContainer = script.Parent.Main.MainFunc.ScriptHub
    local safeName = tostring(name or "Saved.lua")

    -- Refresh an existing saved entry instead of creating duplicates.
    for _, existing in ipairs(tabContainer:GetChildren()) do
        if existing:IsA("TextButton") and existing:GetAttribute("ReSaveSavedFile") == true then
            local existingName = existing:GetAttribute("ReSaveSavedFileName") or existing.Text
            if tostring(existingName) == safeName then
                existing.Text = safeName
                existing:SetAttribute("ReSaveSavedFileSource", tostring(source or ""))
                return existing
            end
        end
    end
    ...
    Button.Name = "Saved_" .. safeName:gsub("[^%w_%-%.]", "_")
    Button.Text = safeName
    Button.Visible = true
    Button:SetAttribute("ReSaveSavedFile", true)
    Button:SetAttribute("ReSaveSavedFileName", safeName)
    Button:SetAttribute("ReSaveSavedFileSource", tostring(source or ""))

    Button.MouseButton1Up:Connect(function()
        if click then return end
        click = true
        task.wait(0.18)
        if click and Button.Parent then
            module:AddTab(safeName, source)
            ReSaveNotify("File " .. safeName .. " Open", 1.7)
        end
        click = false
    end)

    Button.MouseButton2Up:Connect(function()
        local old = Button:FindFirstChild("RClick")
        ...
        rclick.ZIndex = 200
        rclick.Execute.ZIndex = 201
        rclick.Load.ZIndex = 201
        rclick.Delete.ZIndex = 201
        ...
        rclick.Execute.MouseButton1Up:Connect(function()
            ...
            if ok then
                ReSaveNotify("Script Executing", 1.6)
            else
                warn("ReSave saved script execute failed:", err)
                ReSaveNotify("Script Error", 2)
            end
            rclick:Destroy()
        end)
        ...
        task.delay(3, function()
            if rclick and rclick.Parent then
                rclick:Destroy()
            end
        end)
    end)
...
function module:RemoveScriptTab(name)
    local target = tostring(name or "")
    for _, child in ipairs(script.Parent.Main.MainFunc.ScriptHub:GetChildren()) do
        if child:IsA("TextButton") then
            local storedName = child:GetAttribute("ReSaveSavedFileName")
            if (storedName and tostring(storedName) == target)
                or tostring(child.Text) == target then
                child:Destroy()
            end
        end
    end
...
local function C_49()
    ...
    local function GetTextbox()
        for _, textbox in ipairs(buttons.MainFunc.Textboxes:GetChildren()) do
            if textbox.Visible then
                return textbox
            end
        end
    ...
    buttons.Clear.MouseButton1Up:Connect(function()
        ...
        ReSaveLog("INFO", "EDITOR", "Clear pressed", "activeEditorCleared=" .. tostring(cleared))
    end)

    buttons.Execute.MouseButton1Up:Connect(function()
        local injected = root.Injected.Value == true
        ReSaveLog("INFO", "EXECUTE", "Execute pressed", "attached=" .. tostring(injected))

        if not injected then
            title.Text = "ReSave - " .. resaveversion .. " (not injected! press attach)"
            ReSaveNotify("Attach is required before Execute", 2)
            ReSaveLog("WARN", "EXECUTE", "Execution blocked", "reason=not attached")
            return
        end

        local textbox = GetTextbox()
        if not textbox then
            ReSaveNotify("No active script", 1.8)
            ReSaveLog("WARN", "EXECUTE", "Execution blocked", "reason=no active script tab")
            return
        end

        local source = tostring(textbox.Frame.Textbox.Text or "")
        ReSaveLog("INFO", "EXECUTE", "Compiling active editor", "characters=" .. tostring(#source))
        ReSaveNotify("Script Executing", 1.5)

        task.spawn(function()
            local loader = loadstring
            if type(loader) ~= "function" then
                ReSaveLog("ERROR", "EXECUTE", "loadstring is unavailable", "environment does not expose a Luau string compiler")
                ReSaveNotify("Execution API Unavailable", 2.2)
                return
            end

            local chunk, compileErr = loader(source, "ReSave.Editor")
            if type(chunk) ~= "function" then
                local line = ReSaveExtractLine(compileErr)
                local detail = "error=" .. tostring(compileErr)
                if line then
                    detail = detail .. "\nline=" .. tostring(line)
                end
                ReSaveLog("ERROR", "SYNTAX ERROR", "Active script failed compilation", detail)
                ReSaveNotify("Syntax Error", 2.2)
                return
            end

            ReSaveLog("INFO", "EXECUTE", "Syntax check passed", "compiled=true")

            local ok, result = xpcall(chunk, function(err)
                return ReSaveTrace(err)
            end)

            if ok then
                ReSaveLog("INFO", "EXECUTE SUCCESS", "Active script finished without an unhandled error", "resultType=" .. typeof(result))
            else
                local line = ReSaveExtractLine(result)
                local detail = tostring(result)
                if line then
                    detail = detail .. "\nline=" .. tostring(line)
                end
                ReSaveLog("ERROR", "RUNTIME ERROR", "Active script terminated with an error", detail)
                ReSaveNotify("Script Error", 2.2)
            end
        end)
    end)

    buttons.Attach.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "ATTACH", "Attach requested", "previousState=" .. tostring(root.Injected.Value == true))

        if root.Injected.Value ~= true then
            title.Text = "ReSave - " .. resaveversion .. " (checking...)"
            ReSaveLog("INFO", "ATTACH", "Checking environment", "stage=checking")
            task.wait(0.55)
            title.Text = "ReSave - " .. resaveversion .. " (injecting...)"
            ReSaveLog("INFO", "ATTACH", "Preparing client session", "stage=injecting")
            task.wait(0.8)
            title.Text = "ReSave - " .. resaveversion .. " (checking whitelist...)"
            ReSaveLog("INFO", "ATTACH", "Checking simulated compatibility state", "stage=compatibility")
            task.wait(0.65)
            title.Text = "ReSave - " .. resaveversion .. " (scanning...)"
            ReSaveLog("INFO", "ATTACH", "Scanning simulated client state", "stage=scanning")
            task.wait(0.55)

            root.Injected.Value = true
            root:SetAttribute("ReSaveAttached", true)
            title.Text = "ReSave - " .. resaveversion .. " (ready!)"
            ReSaveLog("INFO", "ATTACH SUCCESS", "Attach completed", "attached=true")
            ReSaveNotify("Attach Complete", 1.6)
            task.wait(0.85)
            title.Text = "ReSave - " .. resaveversion
        else
            root:SetAttribute("ReSaveAttached", true)
            title.Text = "ReSave - " .. resaveversion .. " (already attached!)"
            ReSaveLog("INFO", "ATTACH", "Attach pressed while already attached", "attached=true")
            ReSaveNotify("Already Attached", 1.5)
            task.wait(0.85)
            title.Text = "ReSave - " .. resaveversion
        end
    end)
    ...
    buttons.Options.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "OPTIONS", "Options opened")
        root.OptionMenu.Position = UDim2.new(0.5, 0, 0.5, 0)
        root.OptionMenu.Visible = true
    end)

    buttons.OpenFile.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "OPEN FILE", "Open File window opened")
        root.OpenScript.Visible = true
    end)

    buttons.SaveFile.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "SAVE FILE", "Save File window opened")
        local window = root.SaveScript
        window.Visible = true
        window.scriptname:CaptureFocus()
    end)

    buttons.Close.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "UI", "Main window close requested")
        root:Destroy()
    end)

    buttons.Maximize.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "UI", "Maximize button pressed", "feature=reserved")
    end)

    buttons.Minimize.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "UI", "Main window minimized")
        buttons.Visible = false
        root.FloatingIcon.Visible = true
    end)

    root.FloatingIcon.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "UI", "Floating icon pressed; main window restored")
        buttons.Visible = true
        root.FloatingIcon.Visible = false
    end)

    buttons.ScriptHub.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "SCRIPT HUB", "Script Hub opened")
        root.ScriptHubMenu.Visible = true
    end)
end
task.spawn(C_49);
-- StarterGui.ReSave.SaveScript.Handler
local function C_51()
    ...
    saveWindow.savescript.MouseButton1Up:Connect(function()
        ...
        ReSaveLog("INFO", "SAVE FILE", "Save requested", "rawName=" .. (rawName ~= "" and rawName or "<empty>"))

        if not activeTextbox then
            saveWindow.Title.Text = "ReSave - Save File (no editor)"
            ReSaveLog("WARN", "SAVE FILE", "Save aborted", "reason=no active editor")
            ReSaveNotify("Nothing to save", 1.7)
            task.wait(0.9)
        ...
        if not fileName then
            ...
            ReSaveLog("WARN", "SAVE FILE", "Save aborted", "reason=invalid filename")
            ReSaveNotify("Enter a file name", 1.7)
            task.wait(0.9)
            saveWindow.Title.Text = "ReSave - Save File"
        ...
        local source = tostring(activeTextbox.Frame.Textbox.Text or "")
        local folder = "ReSaveSavedScripts"
        local path = folder .. "/" .. fileName

        ReSaveLog("INFO", "SAVE FILE", "Filename normalized", "fileName=" .. fileName .. "\npath=" .. path .. "\ncharacters=" .. tostring(#source))

        local folderOK, folderErr = pcall(function()
            makefolder(folder)
        end)
        if not folderOK then
            ReSaveLog("INFO", "SAVE FILE", "makefolder returned an error; folder may already exist", tostring(folderErr))
        end
        ...
        if not writeOK then
            ...
            ReSaveLog("ERROR", "SAVE FAILED", "writefile failed", "path=" .. path .. "\nerror=" .. tostring(writeErr))
            warn("ReSave save failed:", writeErr or "writefile error")
            ReSaveNotify("Save Failed: " .. fileName, 2.2)
            task.wait(1.1)
            saveWindow.Title.Text = "ReSave - Save File"
            return
        end
        ...
        if not addOK then
            ReSaveLog("ERROR", "SAVE FILE", "Saved successfully but UI list update failed", tostring(addErr))
        else
            ReSaveLog("INFO", "SAVE FILE", "Saved file and refreshed saved-script list", "fileName=" .. fileName)
        end

        ReSaveNotify("Save File: " .. fileName, 1.9)
        saveWindow.scriptname.Text = ""
        saveWindow.Visible = false
    end)

    saveWindow.Close.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "SAVE FILE", "Save window closed without saving")
        saveWindow.Visible = false
        saveWindow.scriptname.Text = ""
    end)
end
...
local function C_71()
    ...
    local background = hub:WaitForChild("Background")
    local list = background:WaitForChild("ScrollingFrame")

    local scripts = {
        Dex = {
            description = "A version of the popular Dex explorer with\npatches specifically for ReSave.",
            preview = "7070160765",
            run = function()
        ...
        RemoteSpy = {
            description = "Allows you to view RemoteEvents and\nRemoteFunctions called.",
            preview = "7370616106",
            run = function()
        ...
        ShiftLock = {
            description = "A lightweight ShiftLock utility from LamScripts.",
            preview = "94251318161622",
            run = function()
    ...
    local orderedNames = {"Dex", "RemoteSpy", "UnnamedESP", "ScriptDumper", "ShiftLock"}
    local selectedName = nil
    local previewGeneration = 0

    local function allGuiVisible()
        hub.Background.Visible = true
        hub.Panel.Visible = true
        hub.Title.Visible = true
        hub.Icon.Visible = true
        hub.Minimize.Visible = true
        hub.Close.Visible = true
        hub.Description.Visible = true
        hub.Execute.Visible = true
    end

    -- Use several Roblox content identifiers in sequence.  The legacy
    -- asset endpoint is deliberately tried first, then asset-id, then thumb.
    -- This is more tolerant of assets whose normal rbxassetid form does not
    -- immediately resolve in an ImageLabel.
    local function loadPreview(frame, assetId)
        if not frame then return end
        local image = frame:FindFirstChild("Image")
        if not image or not (image:IsA("ImageLabel") or image:IsA("ImageButton")) then return end

        previewGeneration += 1
        local myGeneration = previewGeneration
        local assetString = tostring(assetId)
        local isShiftLock = (assetString == "94251318161622")
        ...
        -- ShiftLock uses the modern Content/ImageContent path first. Roblox's
        -- current ImageLabel API exposes ImageContent as the canonical content
        -- property and it accepts asset URIs directly.
        if isShiftLock then
            local uri = "rbxassetid://94251318161622"
            local appliedWithContentApi = false

            pcall(function()
                image.ImageContent = Content.fromUri(uri)
                appliedWithContentApi = true
            end)

            if not appliedWithContentApi then
                pcall(function()
                    image.Image = uri
                end)
            end
            ...
            task.spawn(function()
                ...
                pcall(function()
                    cp:PreloadAsync({image})
                end)
                if myGeneration ~= previewGeneration then return end

                -- Retry with the URI once. No square rbxthumb fallback is used,
                -- because this asset is a 512x277 horizontal image.
        ...
        -- Existing preview behavior for the other hub entries.
        local candidates = {
            "rbxassetid://" .. assetString,
            "http://www.roblox.com/asset/?id=" .. assetString,
            "rbxthumb://type=Asset&id=" .. assetString .. "&w=420&h=420",
        }
    ...
    local function selectScript(name)
        ...
        -- Preview frames: exactly one visible at a time.
        for _, scriptName in ipairs(orderedNames) do
            local frame = hub:FindFirstChild(scriptName)
            if frame and frame:IsA("Frame") then
                frame.Visible = (scriptName == name)
            end
        end

        hub.Description.Text = data.description
        hub.Description.Visible = true
        hub.Execute.Visible = true
        loadPreview(hub:FindFirstChild(name), data.preview)
    end

    -- Rebuild the script list using layout order only. This prevents the
    -- generated absolute positions from making items vanish/off-screen.
    do
        list.Visible = true
        list.Active = true
        ...
        local padding = list:FindFirstChild("ReSaveHubListPadding")
        if not padding then
            padding = Instance.new("UIPadding")
            padding.Name = "ReSaveHubListPadding"
            padding.Parent = list
        end
        ...
        for index, name in ipairs(orderedNames) do
            local button = list:FindFirstChild(name)
            if button and button:IsA("TextButton") then
                ...
                button.BackgroundColor3 = Color3.fromRGB(19, 17, 25)
                ...
                local p = button:FindFirstChild("ReSaveHubButtonPadding")
                if not p then
                    p = Instance.new("UIPadding")
                    p.Name = "ReSaveHubButtonPadding"
                    p.Parent = button
                end
                p.PaddingLeft = UDim.new(0, 9)

                if not button:GetAttribute("ReSaveBound") then
                    button:SetAttribute("ReSaveBound", true)
                    button.MouseButton1Up:Connect(function()
                        ReSaveLog("INFO", "SCRIPT HUB", "Script selected", "name=" .. name)
                        selectScript(name)
                    end)
                end
            end
    ...
    if not hub:GetAttribute("ReSaveHubExecuteBound") then
        hub:SetAttribute("ReSaveHubExecuteBound", true)
        hub.Execute.MouseButton1Up:Connect(function()
            local data = selectedName and scripts[selectedName]
            if not data or type(data.run) ~= "function" then
                ReSaveLog("WARN", "SCRIPT HUB", "Execute pressed with no valid selection")
                return
            end

            local scriptName = tostring(selectedName)
            ReSaveLog("INFO", "SCRIPT HUB EXECUTE", "Executing Script Hub item", "name=" .. scriptName)
            ReSaveNotify("Script Executing: " .. scriptName, 1.6)

            task.spawn(function()
                local ok, err = xpcall(data.run, function(runtimeErr)
                    return ReSaveTrace(runtimeErr)
                end)
                if ok then
                    ReSaveLog("INFO", "SCRIPT HUB SUCCESS", "Script Hub item finished without an unhandled error", "name=" .. scriptName)
                else
                    ReSaveLog("ERROR", "SCRIPT HUB ERROR", "Script Hub item failed", "name=" .. scriptName .. "\nerror=" .. tostring(err))
                    ReSaveNotify("Script Hub Error", 2.2)
                end
            end)
        end)
    end

    if not hub:GetAttribute("ReSaveHubCloseBound") then
        hub:SetAttribute("ReSaveHubCloseBound", true)
        hub.Close.MouseButton1Up:Connect(function()
            ReSaveLog("INFO", "SCRIPT HUB", "Script Hub closed")
            hub.Visible = false
        end)
    end

    if not hub:GetAttribute("ReSaveHubMinimizeBound") then
        hub:SetAttribute("ReSaveHubMinimizeBound", true)
        hub.Minimize.MouseButton1Up:Connect(function()
            ReSaveLog("INFO", "SCRIPT HUB", "Script Hub minimized/hidden")
            hub.Visible = false
        end)
    end
...
local function C_83()
	...
	stuff.Close.MouseButton1Up:Connect(function()
		ReSaveLog("INFO", "OPTIONS", "Options closed")
	...
	stuff.ToggleAutoExec.MouseButton1Up:Connect(function()
		ReSaveLog("INFO", "OPTIONS", "Auto Exec toggled", "enabled=" .. tostring(stuff.ToggleAutoExec.Text == ''))
	...
	stuff.ToggleFPSUnlocker.MouseButton1Up:Connect(function()
		ReSaveLog("INFO", "OPTIONS", "FPS Unlocker toggled", "enabledBefore=" .. tostring(stuff.ToggleFPSUnlocker.Text == ''))
	...
	stuff.ToggleInternalUI.MouseButton1Up:Connect(function()
		ReSaveLog("INFO", "OPTIONS", "Internal UI toggled", "enabledBefore=" .. tostring(stuff.ToggleInternalUI.Text == ''))
	...
	stuff.ToggleTopMost.MouseButton1Up:Connect(function()
		ReSaveLog("INFO", "OPTIONS", "TopMost toggled", "enabledBefore=" .. tostring(stuff.ToggleTopMost.Text == ''))
...
local function C_84()
    ...
    ReSaveLog("INFO", "STARTUP", "Loading saved scripts", "folder=ReSaveSavedScripts")
    ...
    ReSaveLog("INFO", "STARTUP", "Saved scripts loaded", "count=" .. tostring(fileCount))
end;
task.spawn(C_84);
-- StarterGui.ReSave.OpenScript.Handler
local function C_92()
    ...
    local module = require(root.Module)
    ...
    local function getSavedFiles()
        ...
        for _, path in ipairs(files) do
            local checkOK, isValid = pcall(function()
                return isfile(path)
            end)
    ...
    local function ensureConfirmModal()
        local modal = root:FindFirstChild("DeleteConfirm")
        ...
        modal.Size = UDim2.new(0, 350, 0, 142)
        modal.BackgroundColor3 = Color3.fromRGB(14, 12, 19)
        ...
        title.TextSize = 15
        title.TextColor3 = Color3.fromRGB(244, 241, 252)
        ...
        message.Position = UDim2.new(0, 14, 0, 42)
        message.Size = UDim2.new(1, -28, 0, 43)
        message.Font = Enum.Font.Gotham
        message.TextSize = 13
        message.TextColor3 = Color3.fromRGB(183, 174, 199)
        ...
        cancel.Name = "Cancel"
        cancel.Position = UDim2.new(0, 14, 1, -43)
        cancel.Size = UDim2.new(0.5, -20, 0, 30)
        cancel.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
        cancel.BorderSizePixel = 0
        cancel.Text = "Cancel"
        cancel.TextColor3 = Color3.fromRGB(244, 241, 252)
        cancel.TextSize = 13
        ...
        confirm.Position = UDim2.new(0.5, 6, 1, -43)
        confirm.Size = UDim2.new(0.5, -20, 0, 30)
        confirm.BackgroundColor3 = Color3.fromRGB(86, 45, 115)
        confirm.BorderSizePixel = 0
        confirm.Text = "🗑️ Delete"
    ...
    local confirmModal = ensureConfirmModal()
    ...
    local function openConfirm(path, fileName)
        pendingPath = path
        pendingFileName = fileName
        confirmModal.Message.Text = 'Delete "' .. fileName .. '"? This cannot be undone.'
        confirmModal.Visible = true
        ReSaveLog("INFO", "DELETE FILE", "Delete confirmation opened", "fileName=" .. fileName .. "\npath=" .. tostring(path))
    end
    ...
    local function rebuildList()
        ...
        for _, child in ipairs(list:GetChildren()) do
            if child:IsA("GuiButton") or child:IsA("TextLabel") or child:IsA("Frame") then
                if child.Name ~= "ReSaveHubListPadding" and not child:IsA("UIPadding") then
                    child:Destroy()
                end
            end
        end

        local results = getSavedFiles()
        if #results == 0 then
        ...
        for index, path in ipairs(results) do
            ...
            row.Name = "FileRow_" .. tostring(index)
            ...
            openButton.Size = UDim2.new(1, -40, 1, 0)
            openButton.BackgroundColor3 = Color3.fromRGB(25, 20, 33)
            openButton.BorderSizePixel = 0
            openButton.Text = fileName
            openButton.TextColor3 = Color3.fromRGB(244, 241, 252)
            openButton.TextSize = 13
            openButton.Font = Enum.Font.Gotham
            openButton.TextXAlignment = Enum.TextXAlignment.Left
            openButton.AutoButtonColor = false
            openButton.ZIndex = 113
            openButton.Parent = row

            local openPad = Instance.new("UIPadding")
            openPad.PaddingLeft = UDim.new(0, 10)
            openPad.Parent = openButton
            ...
            deleteButton.Position = UDim2.new(1, -34, 0, 0)
            deleteButton.Size = UDim2.new(0, 34, 1, 0)
            deleteButton.BackgroundColor3 = Color3.fromRGB(31, 22, 39)
            deleteButton.BorderSizePixel = 0
            deleteButton.Text = "🗑️"
            deleteButton.TextSize = 16
            deleteButton.Font = Enum.Font.Gotham
            deleteButton.TextColor3 = Color3.fromRGB(244, 241, 252)
            deleteButton.AutoButtonColor = false
            deleteButton.ZIndex = 114
            ...
            local function setHover(button, normal, hover)
                button.MouseEnter:Connect(function()
                    if button.Parent then button.BackgroundColor3 = hover end
                end)
                button.MouseLeave:Connect(function()
                    if button.Parent then button.BackgroundColor3 = normal end
                end)
            end
            setHover(openButton, Color3.fromRGB(25, 20, 33), Color3.fromRGB(54, 34, 72))
            setHover(deleteButton, Color3.fromRGB(31, 22, 39), Color3.fromRGB(76, 40, 90))

            openButton.MouseButton1Up:Connect(function()
                ReSaveLog("INFO", "OPEN FILE", "Open requested", "fileName=" .. fileName .. "\npath=" .. tostring(path))
                ...
                if readOK then
                    ...
                    ReSaveLog("INFO", "OPEN FILE SUCCESS", "File opened", "fileName=" .. fileName .. "\ncharacters=" .. tostring(#tostring(source)))
                    ReSaveNotify("File " .. fileName .. " Open", 1.8)
                else
                    window.Title.Text = "ReSave - Open File (read failed)"
                    ReSaveLog("ERROR", "OPEN FILE FAILED", "readfile failed", "fileName=" .. fileName .. "\npath=" .. tostring(path) .. "\nerror=" .. tostring(source))
                    ReSaveNotify("Open Failed: " .. fileName, 2)
            ...
            deleteButton.MouseButton1Up:Connect(function()
                openConfirm(path, fileName)
            end)
        end
    end

    confirmModal.Cancel.MouseButton1Up:Connect(function()
        ReSaveLog("INFO", "DELETE FILE", "Delete confirmation cancelled", "fileName=" .. tostring(pendingFileName or "<none>"))
        closeConfirm()
    end)

    confirmModal.Confirm.MouseButton1Up:Connect(function()
        if confirmBusy or not pendingPath or not pendingFileName then return end
        confirmBusy = true

        local path = pendingPath
        local fileName = pendingFileName
        local existsOK, exists = pcall(function()
            return isfile(path)
        end)

        ReSaveLog("INFO", "DELETE FILE", "Delete confirmed", "fileName=" .. fileName .. "\npath=" .. tostring(path))

        if not existsOK or not exists then
            ReSaveLog("WARN", "DELETE FILE", "Delete aborted because file is missing", "fileName=" .. fileName .. "\nexistsCheckError=" .. tostring(exists))
            ReSaveNotify("File already missing: " .. fileName, 1.9)
            closeConfirm()
            rebuildList()
            return
        end

        local deleteOK, deleteErr = false, nil
        if type(delfile) == "function" then
            deleteOK, deleteErr = pcall(function()
                delfile(path)
            end)
        else
            deleteErr = "delfile is unavailable"
        end

        if deleteOK then
            local removeOK, removeErr = pcall(function()
                module:RemoveScriptTab(fileName)
            end)
            ReSaveLog("INFO", "DELETE FILE SUCCESS", "File deleted", "fileName=" .. fileName .. "\npath=" .. tostring(path) .. "\nuiEntryRemoved=" .. tostring(removeOK))
            if not removeOK then
                ReSaveLog("WARN", "DELETE FILE", "Disk delete succeeded but UI entry removal failed", tostring(removeErr))
            end
            ReSaveNotify("Deleted: " .. fileName, 1.8)
        else
            warn("ReSave delete failed:", deleteErr or "delfile error")
            ReSaveLog("ERROR", "DELETE FILE FAILED", "delfile failed", "fileName=" .. fileName .. "\npath=" .. tostring(path) .. "\nerror=" .. tostring(deleteErr or "delfile error"))
            ReSaveNotify("Delete Failed: " .. fileName, 2.2)
        end
    ...
    window:GetPropertyChangedSignal("Visible"):Connect(function()
        if window.Visible then rebuildList() end
    end)
...
do
	...
	if toolbar and not toolbar:IsA("ScrollingFrame") then
		toolbar.Name = "ReSaveToolbar_Old"
		toolbar = nil
	end
	if not toolbar then
		toolbar = Instance.new("ScrollingFrame")
		toolbar.Name = "ReSaveToolbar"
		toolbar.Parent = main
	end

	-- Keep the normal button size. On small screens the row scrolls horizontally
	-- instead of shrinking every button just to fit the new Log action.
	...
	toolbar.ScrollBarImageTransparency = 0.15
	toolbar.CanvasPosition = Vector2.new(0, 0)
	toolbar.CanvasSize = UDim2.new(0, 0, 0, 0)
	toolbar.AutomaticCanvasSize = Enum.AutomaticSize.None

	local content = toolbar:FindFirstChild("ActionContent")
	if not content then
		content = Instance.new("Frame")
		content.Name = "ActionContent"
		content.Parent = toolbar
	end
	content.BackgroundTransparency = 1
	content.BorderSizePixel = 0
	content.Position = UDim2.new(0, 6, 0, 2)
	content.Size = UDim2.new(0, 705, 0, 32)
	content.ZIndex = 19
	...
	local totalWidth = (#actionNames * buttonWidth) + ((#actionNames - 1) * buttonGap)
	content.Size = UDim2.new(0, totalWidth, 0, 32)
	toolbar.CanvasSize = UDim2.new(0, totalWidth + 12, 0, 32)

	local function styleActionButton(button, name, index)
		button.Parent = content
		button.Position = UDim2.new(0, (index - 1) * (buttonWidth + buttonGap), 0, 0)
		...
		corner(button, 7)
		stroke(button, C.stroke, 1, 0.1)

		local label = button:FindFirstChild("NameText")
		if label then
		...
		if name == "Execute" or name == "Attach" then
			...
			gradient(button, Color3.fromRGB(67, 38, 91), Color3.fromRGB(33, 22, 43), 90)
		else
			gradient(button, Color3.fromRGB(27, 22, 34), Color3.fromRGB(15, 13, 19), 90)
		end
	end

	-- Log is a real action button in the same row as Execute/Script Hub.
	local logButton = main:FindFirstChild("Log")
	if logButton and not logButton:IsA("TextButton") then
		logButton:Destroy()
		logButton = nil
	end
	...
	logButton.Text = "Log"
	logButton.Font = Enum.Font.GothamMedium
	logButton.TextSize = 13
	logButton.TextColor3 = C.text
	logButton.AutoButtonColor = false

	for index, name in ipairs(actionNames) do
		local button = (name == "Log") and logButton or main:FindFirstChild(name)
		if button then
			styleActionButton(button, name, index)
		end
	end

	-- A fresh console is used so the feature is not tied to the old hidden
	-- ScriptLog layout from the original UI.
	local logConsole = root:FindFirstChild("ReSaveLogConsole")
	if not logConsole then
		logConsole = Instance.new("Frame")
		logConsole.Name = "ReSaveLogConsole"
		logConsole.Parent = root
	end
	logConsole.Visible = false
	logConsole.Active = true
	logConsole.AnchorPoint = Vector2.new(0.5, 0.5)
	logConsole.Position = UDim2.new(0.5, 0, 0.5, 0)
	logConsole.Size = UDim2.new(0, 560, 0, 350)
	logConsole.BackgroundColor3 = C.top
	logConsole.BackgroundTransparency = 0
	logConsole.BorderSizePixel = 0
	logConsole.ZIndex = 800
	logConsole.ClipsDescendants = true
	corner(logConsole, 9)
	stroke(logConsole, C.stroke, 1, 0.05)

	local logHeader = logConsole:FindFirstChild("Header")
	if not logHeader then
		logHeader = Instance.new("Frame")
		logHeader.Name = "Header"
		logHeader.Parent = logConsole
	end
	logHeader.Position = UDim2.new(0, 1, 0, 1)
	logHeader.Size = UDim2.new(1, -2, 0, 34)
	logHeader.BackgroundColor3 = C.panel
	logHeader.BackgroundTransparency = 0
	logHeader.BorderSizePixel = 0
	logHeader.ZIndex = 801

	local logTitle = logHeader:FindFirstChild("Title")
	if not logTitle then
		logTitle = Instance.new("TextLabel")
		logTitle.Name = "Title"
		logTitle.Parent = logHeader
	end
	logTitle.BackgroundTransparency = 1
	logTitle.Position = UDim2.new(0, 12, 0, 0)
	logTitle.Size = UDim2.new(1, -150, 1, 0)
	logTitle.Font = Enum.Font.GothamSemibold
	logTitle.Text = "Log Console"
	logTitle.TextColor3 = C.text
	logTitle.TextSize = 14
	logTitle.TextXAlignment = Enum.TextXAlignment.Left
	logTitle.ZIndex = 802

	local logClear = logHeader:FindFirstChild("Clear")
	if not logClear then
		logClear = Instance.new("TextButton")
		logClear.Name = "Clear"
		logClear.Parent = logHeader
	end
	logClear.Text = "Clear"
	logClear.Font = Enum.Font.GothamMedium
	logClear.TextSize = 12
	logClear.TextColor3 = C.text
	logClear.BackgroundColor3 = C.editor
	logClear.BorderColor3 = C.purple2
	logClear.BorderSizePixel = 1
	logClear.BorderSizePixel = 0
	logClear.Position = UDim2.new(1, -110, 0, 5)
	logClear.Size = UDim2.new(0, 58, 0, 24)
	logClear.ZIndex = 802
	corner(logClear, 5)

	local logClose = logHeader:FindFirstChild("Close")
	if not logClose then
		logClose = Instance.new("TextButton")
		logClose.Name = "Close"
		logClose.Parent = logHeader
	end
	logClose.Text = "×"
	logClose.Font = Enum.Font.GothamSemibold
	logClose.TextSize = 16
	logClose.TextColor3 = C.text
	logClose.BackgroundColor3 = C.editor
	logClose.BorderSizePixel = 0
	logClose.Position = UDim2.new(1, -44, 0, 5)
	logClose.Size = UDim2.new(0, 30, 0, 24)
	logClose.ZIndex = 802
	corner(logClose, 5)

	local logList = logConsole:FindFirstChild("LogList")
	if not logList then
		logList = Instance.new("ScrollingFrame")
		logList.Name = "LogList"
		logList.Parent = logConsole
	end
	logList.Position = UDim2.new(0, 8, 0, 42)
	logList.Size = UDim2.new(1, -16, 1, -50)
	logList.BackgroundColor3 = C.editor
	logList.BorderSizePixel = 0
	logList.Active = true
	logList.ScrollBarThickness = 5
	logList.ScrollingDirection = Enum.ScrollingDirection.Y
	logList.AutomaticCanvasSize = Enum.AutomaticSize.Y
	logList.CanvasSize = UDim2.new(0, 0, 0, 0)
	logList.ZIndex = 801
	corner(logList, 6)

	local logLayout = logList:FindFirstChild("Layout")
	...
	local logPadding = logList:FindFirstChild("Padding")
	if not logPadding then
		logPadding = Instance.new("UIPadding")
		logPadding.Name = "Padding"
	...
	local function renderLogConsole()
		for _, child in ipairs(logList:GetChildren()) do
			if child:IsA("TextLabel") and child:GetAttribute("ReSaveLogRow") == true then
				child:Destroy()
			end
		end
		...
		local colorMap = {
			...
			INFO = Color3.fromRGB(215, 202, 255),
			OUTPUT = Color3.fromRGB(190, 190, 205),
		}

		for _, entry in ipairs(ReSaveLogEntries) do
			local row = Instance.new("TextLabel")
			row.Name = "LogEntry_" .. tostring(entry.id)
			row:SetAttribute("ReSaveLogRow", true)
			...
			row.TextSize = 12
			row.TextColor3 = colorMap[entry.level] or colorMap.INFO
			row.ZIndex = 803
		...
		task.defer(function()
			if logList.Parent then
				logList.CanvasPosition = Vector2.new(
					0,
					math.max(0, logList.AbsoluteCanvasSize.Y - logList.AbsoluteSize.Y)
				)
			end
		end)
	end

	ReSaveLogRender = renderLogConsole

	if not logButton:GetAttribute("ReSaveLogBound24") then
		logButton:SetAttribute("ReSaveLogBound24", true)
		logButton.Activated:Connect(function()
			logConsole.Visible = not logConsole.Visible
			ReSaveLog("INFO", "LOG CONSOLE", logConsole.Visible and "Log Console opened" or "Log Console closed")
			renderLogConsole()
		end)
	end

	if not logClear:GetAttribute("ReSaveLogClearBound24") then
		logClear:SetAttribute("ReSaveLogClearBound24", true)
		logClear.Activated:Connect(function()
			local oldSize = logClear.Size
			local oldPos = logClear.Position

			-- Clear the current buffer, then immediately record the clear action as
			-- the first fresh entry so the action itself is visible in the console.
			table.clear(ReSaveLogEntries)
			ReSaveLogSequence = 0
			ReSaveLogLastFingerprint = nil
			ReSaveLogLastClock = 0
			ReSaveLog("INFO", "LOG CONSOLE", "Log cleared", "entriesBeforeClear=buffer reset")

			-- Small press animation without changing layout permanently.
			...
			pressTween.Completed:Connect(function()
				if logClear.Parent then
					local restoreTween = TweenService:Create(
						logClear,
						TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
						{BackgroundTransparency = 0, Size = oldSize, Position = oldPos}
					)
					restoreTween:Play()
				end
			end)
	...
	if not logClose:GetAttribute("ReSaveLogCloseBound24") then
		logClose:SetAttribute("ReSaveLogCloseBound24", true)
		logClose.Activated:Connect(function()
			logConsole.Visible = false
			ReSaveLog("INFO", "LOG CONSOLE", "Log Console closed")
		end)
	end

	renderLogConsole()

	-- Attach highlight must disappear completely once the simulated Attach state
	-- becomes true. A watchdog also removes any stale stroke left by old builds.
	local attachButton = main:FindFirstChild("Attach")
	if attachButton then
		local TweenService = game:GetService("TweenService")
		local pulseToken = 0

		local function isAttached()
			return root.Injected.Value == true or root:GetAttribute("ReSaveAttached") == true
		end

		local function removeAttachHighlight()
			pulseToken += 1
			local pulse = attachButton:FindFirstChild("ReSaveAttachPulse")
			if pulse then
				pulse:Destroy()
			end
			attachButton:SetAttribute("ReSaveAttachHighlightActive", false)
			attachButton.BackgroundColor3 = C.purpleDark
		end

		local function addAttachHighlight()
			if isAttached() then
				removeAttachHighlight()
				return
			end

			local pulse = attachButton:FindFirstChild("ReSaveAttachPulse")
			if not pulse then
				pulse = Instance.new("UIStroke")
				pulse.Name = "ReSaveAttachPulse"
				pulse.Parent = attachButton
			end
			pulse.Color = C.purple2
			pulse.Thickness = 1.7
			pulse.Transparency = 0.03
			attachButton:SetAttribute("ReSaveAttachHighlightActive", true)

			pulseToken += 1
			local token = pulseToken
			task.spawn(function()
				while attachButton.Parent and token == pulseToken do
					if isAttached() then
						removeAttachHighlight()
						return
					end
					local current = attachButton:FindFirstChild("ReSaveAttachPulse")
					if not current then return end
					TweenService:Create(
						current,
						TweenInfo.new(0.55, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
						{Transparency = 0.5, Thickness = 2.2}
					):Play()
					task.wait(0.55)
					if isAttached() then
						removeAttachHighlight()
					...
					current = attachButton:FindFirstChild("ReSaveAttachPulse")
					if not current then return end
					TweenService:Create(
						current,
						TweenInfo.new(0.55, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
						{Transparency = 0.03, Thickness = 1.7}
					):Play()
					task.wait(0.55)
				end
			end)
		...
		root.Injected:GetPropertyChangedSignal("Value"):Connect(function()
			if isAttached() then
				removeAttachHighlight()
				ReSaveLog("INFO", "ATTACH", "Attach confirmed; highlight removed", "highlight=false")
			else
				addAttachHighlight()
			end
		end)

		root:GetAttributeChangedSignal("ReSaveAttached"):Connect(function()
			if isAttached() then
				removeAttachHighlight()
			else
				addAttachHighlight()
	...
	ReSaveLog(
		"INFO",
		"STARTUP",
		"ReSave v2.26.upg2.4 initialized",
		"Horizontal action scroller=ready\nLog Console=ready\nAttach highlight=state-bound"
	)

	local hubModalForTheme = root:FindFirstChild("ScriptHubMenu")
	local shiftButton = hubModalForTheme and hubModalForTheme.Background.ScrollingFrame:FindFirstChild("ShiftLock")
	if shiftButton and shiftButton:IsA("TextButton") then
		shiftButton.TextColor3 = C.text
		shiftButton.BackgroundColor3 = C.editor
		shiftButton.TextXAlignment = Enum.TextXAlignment.Left
		shiftButton.TextSize = 13
		shiftButton.ZIndex = 104
	end
	...
	if saveModal then
		...
		saveModal.scriptname.PlaceholderText = "File name (.lua is optional)"
	...
	if hubModal then
		...
		local shiftLockFrame = hubModal:FindFirstChild("ShiftLock")
		local shiftLockImage = shiftLockFrame and shiftLockFrame:FindFirstChild("Image")
		if shiftLockImage and (shiftLockImage:IsA("ImageLabel") or shiftLockImage:IsA("ImageButton")) then
			shiftLockImage.Visible = true
			shiftLockImage.Image = "rbxassetid://94251318161622"
			shiftLockImage.ImageTransparency = 0
			shiftLockImage.ImageColor3 = Color3.fromRGB(255, 255, 255)
			shiftLockImage.ScaleType = Enum.ScaleType.Fit
			pcall(function()
				game:GetService("ContentProvider"):PreloadAsync({shiftLockImage})
			end)
			task.delay(1, function()
				if shiftLockImage and shiftLockImage.Parent and shiftLockImage.IsLoaded == false then
					shiftLockImage.Image = "rbxthumb://type=Asset&id=94251318161622&w=420&h=420"
				end
			end)
		end
	end
...
-- ============================================================
-- ReSave Script Hub + rounded-window repair pass.
-- This deliberately runs last so it overrides legacy generated layout values.
-- ============================================================
do
	local root = G2L["1"]
	local main = G2L["6"]
	if root and main then
		local function addCorner(obj, radius)
			...
			local c = obj:FindFirstChild("ReSaveFinalCorner")
			if not c then
				c = Instance.new("UICorner")
				c.Name = "ReSaveFinalCorner"
				c.Parent = obj
			end
			c.CornerRadius = UDim.new(0, radius)
		end
		...
		local function addStroke(obj, color, thickness, transparency)
			if not obj then return end
			local s = obj:FindFirstChild("ReSaveFinalStroke")
			if not s then
				s = Instance.new("UIStroke")
				s.Name = "ReSaveFinalStroke"
				s.Parent = obj
			end
			s.Color = color
			s.Thickness = thickness
		...
		-- Main surface: keep all child surfaces slightly inset so the rounded
		-- corners cannot be visually overpainted by square child backgrounds.
		main.ClipsDescendants = true
		main.BorderSizePixel = 0
		addCorner(main, 10)
		addStroke(main, Color3.fromRGB(151, 94, 246), 1.5, 0.22)
		...
		if bg then
			...
			bg.BackgroundTransparency = 0
			bg.ImageTransparency = 1
			bg.BackgroundColor3 = Color3.fromRGB(8, 7, 11)
			addCorner(bg, 9)
		end
		...
		local toolbar = main:FindFirstChild("ReSaveToolbar")
		if toolbar then
			toolbar.Position = UDim2.new(0, 1, 0, 242)
			toolbar.Size = UDim2.new(1, -2, 0, 40)
			toolbar.ZIndex = 18
			toolbar.ClipsDescendants = true
			addCorner(toolbar, 9)
		end

		for _, name in ipairs({"Execute","Clear","OpenFile","SaveFile","Options","Attach","ScriptHub"}) do
			local b = main:FindFirstChild(name)
			if b then
				b.ZIndex = 60
				b.Active = true
				b.Position = UDim2.new(0, ({Execute=7,Clear=96,OpenFile=185,SaveFile=274,Options=363,Attach=452,ScriptHub=541})[name], 0, 247)
				b.Size = UDim2.new(0, 82, 0, 30)
				addCorner(b, 7)
				local label = b:FindFirstChild("NameText")
				if label then label.ZIndex = 62 end
			end
		end

		-- Script Hub: rebuild the whole visible layout instead of trying to
		-- preserve the legacy off-screen positions from the original source.
		local hub = root:FindFirstChild("ScriptHubMenu")
		if hub then
			hub.Active = true
			hub.Draggable = false
			hub.Size = UDim2.new(0, 420, 0, 290)
			hub.Position = UDim2.new(0.5, 0, 0.5, 0)
			hub.AnchorPoint = Vector2.new(0.5, 0.5)
			hub.BackgroundColor3 = Color3.fromRGB(14, 12, 19)
			hub.BackgroundTransparency = 0
			hub.BorderSizePixel = 0
			hub.ClipsDescendants = true
			hub.ZIndex = 100
			addCorner(hub, 10)
			addStroke(hub, Color3.fromRGB(151, 94, 246), 1.2, 0.18)

			local bg2 = hub:FindFirstChild("Background")
			if bg2 then
				bg2.Position = UDim2.new(0, 1, 0, 1)
				bg2.Size = UDim2.new(1, -2, 1, -2)
				bg2.BackgroundColor3 = Color3.fromRGB(18, 16, 23)
				bg2.BackgroundTransparency = 0
				bg2.ImageTransparency = 1
				bg2.ZIndex = 100
				addCorner(bg2, 9)
			end

			local list = bg2 and bg2:FindFirstChild("ScrollingFrame")
			if list then
				list.Position = UDim2.new(0, 10, 0, 40)
				list.Size = UDim2.new(0, 106, 0, 236)
				list.BackgroundColor3 = Color3.fromRGB(13, 12, 17)
				...
				local order = {"Dex","RemoteSpy","UnnamedESP","ScriptDumper","ShiftLock"}
				for i, name in ipairs(order) do
					local btn = list:FindFirstChild(name)
					if btn and btn:IsA("TextButton") then
						btn.Position = UDim2.new(0, 0, 0, 0)
						btn.Size = UDim2.new(1, 0, 0, 31)
						btn.LayoutOrder = i
						btn.BackgroundTransparency = 0
						btn.BackgroundColor3 = Color3.fromRGB(19, 17, 25)
						btn.TextColor3 = Color3.fromRGB(244, 241, 252)
						...
						btn.Active = true
						addCorner(btn, 5)
						local pad = btn:FindFirstChild("ReSaveHubPadding")
						if not pad then
							pad = Instance.new("UIPadding")
							pad.Name = "ReSaveHubPadding"
							pad.Parent = btn
						end
			...
			local title = hub:FindFirstChild("Title")
			if title then
			...
			local icon = hub:FindFirstChild("Icon")
			if icon then
				icon.ZIndex = 132
			...
			local header = hub:FindFirstChild("Panel")
			if header then
				header.Position = UDim2.new(0, 1, 0, 1)
				header.Size = UDim2.new(1, -2, 0, 29)
				header.ZIndex = 120
				header.BackgroundColor3 = Color3.fromRGB(20, 16, 27)
				header.BackgroundTransparency = 0
				addCorner(header, 9)
			end

			-- A dedicated drag strip moves the entire ScriptHubMenu, not the title label.
			local dragBar = hub:FindFirstChild("ReSaveHubDragBar")
			if not dragBar then
				dragBar = Instance.new("Frame")
				dragBar.Name = "ReSaveHubDragBar"
				dragBar.Parent = hub
			end
			dragBar.Position = UDim2.new(0, 40, 0, 0)
			dragBar.Size = UDim2.new(1, -86, 0, 30)
			dragBar.BackgroundTransparency = 1
			dragBar.BorderSizePixel = 0
			dragBar.Active = true
			dragBar.ZIndex = 135

			-- Keep close/minimize above every preview image.
			local minus = hub:FindFirstChild("Minimize")
			if minus then
				minus.Draggable = false
				minus.Active = true
				minus.ZIndex = 150
				minus.Position = UDim2.new(1, -54, 0, 4)
				minus.Size = UDim2.new(0, 22, 0, 22)
				minus.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
				minus.BackgroundTransparency = 0
				minus.Text = "_"
				minus.TextColor3 = Color3.fromRGB(244, 241, 252)
				addCorner(minus, 5)
			end

			local close = hub:FindFirstChild("Close")
			if close then
				close.Draggable = false
				close.Active = true
				close.Visible = true
				close.ZIndex = 150
				close.Position = UDim2.new(0, 257, 1, -37)
				close.Size = UDim2.new(0, 121, 0, 28)
				close.Text = "Close"
				close.TextColor3 = Color3.fromRGB(244, 241, 252)
				close.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
				close.BackgroundTransparency = 0
				addCorner(close, 6)
			end

			local desc = hub:FindFirstChild("Description")
			if desc then
				desc.Visible = true
				desc.ZIndex = 140
				desc.Position = UDim2.new(0, 125, 0, 180)
				desc.Size = UDim2.new(0, 275, 0, 57)
				desc.BackgroundColor3 = Color3.fromRGB(18, 16, 23)
				desc.BackgroundTransparency = 0
				desc.TextColor3 = Color3.fromRGB(244, 241, 252)
				desc.TextXAlignment = Enum.TextXAlignment.Left
				desc.TextYAlignment = Enum.TextYAlignment.Top
				desc.TextSize = 13
				addCorner(desc, 6)
				local pad = desc:FindFirstChild("ReSaveDescPadding")
				if not pad then
					pad = Instance.new("UIPadding")
					pad.Name = "ReSaveDescPadding"
					pad.Parent = desc
				end
				pad.PaddingLeft = UDim.new(0, 9)
			...
			local execute = hub:FindFirstChild("Execute")
			if execute then
				execute.Visible = true
				execute.ZIndex = 150
				execute.Position = UDim2.new(0, 125, 1, -37)
				execute.Size = UDim2.new(0, 121, 0, 28)
				execute.BackgroundColor3 = Color3.fromRGB(63, 38, 86)
				execute.TextColor3 = Color3.fromRGB(244, 241, 252)
				execute.BackgroundTransparency = 0
				addCorner(execute, 6)
			end

			-- All previews share one well-defined rectangle. Only the selected
			-- frame is visible; this prevents old off-canvas positions from leaking.
			local previewNames = {"Dex","RemoteSpy","UnnamedESP","ScriptDumper","ShiftLock"}
			for _, name in ipairs(previewNames) do
				local frame = hub:FindFirstChild(name)
				if frame and frame:IsA("Frame") then
					frame.Position = UDim2.new(0, 125, 0, 48)
					frame.Size = UDim2.new(0, 275, 0, 126)
			...
			local shiftFrame = hub:FindFirstChild("ShiftLock")
			local shiftImage = shiftFrame and shiftFrame:FindFirstChild("Image")
			if shiftFrame and shiftImage and (shiftImage:IsA("ImageLabel") or shiftImage:IsA("ImageButton")) then
				-- Layout only. Actual ShiftLock asset loading is handled by the
				-- final Content/ImageContent repair pass below.
				shiftFrame.Position = UDim2.new(0, 125, 0, 48)
				shiftFrame.Size = UDim2.new(0, 275, 0, 149)
				shiftImage.Position = UDim2.new(0, 0, 0, 0)
				shiftImage.Size = UDim2.new(1, 0, 1, 0)
				shiftImage.Visible = false
				shiftImage.BackgroundTransparency = 1
				shiftImage.ImageTransparency = 0
				shiftImage.ImageColor3 = Color3.fromRGB(255,255,255)
				shiftImage.ScaleType = Enum.ScaleType.Fit
			end

			-- Remove all legacy Draggable flags that can move individual children.
			for _, d in ipairs(hub:GetDescendants()) do
				if d:IsA("GuiObject") and d ~= dragBar then
					d.Draggable = false
				end
			end

			-- Drag the whole ScriptHubMenu using the dedicated header strip.
...
-- ============================================================
-- ReSave final Script Hub + Options repair pass.
-- Runs last so the two popup UIs keep a deterministic layout.
-- ============================================================
do
    local root = G2L["1"]
    ...
    local function setCorner(obj, radius)
        if not obj then return end
        local corner = obj:FindFirstChild("ReSaveFinalCorner")
        if not corner then
            corner = Instance.new("UICorner")
            corner.Name = "ReSaveFinalCorner"
            corner.Parent = obj
        end
    ...
    local function setStroke(obj, color, thickness, transparency)
        if not obj then return end
        local stroke = obj:FindFirstChild("ReSaveFinalStroke")
        if not stroke then
            stroke = Instance.new("UIStroke")
            stroke.Name = "ReSaveFinalStroke"
            stroke.Parent = obj
        end
        stroke.Color = color
    ...
    if hub then
        ...
        hub.ZIndex = 100
        setCorner(hub, 10)
        setStroke(hub, Color3.fromRGB(151, 94, 246), 1.2, 0.18)

        local bg = hub:FindFirstChild("Background")
        if bg then
            bg.Position = UDim2.new(0, 1, 0, 1)
            bg.Size = UDim2.new(1, -2, 1, -2)
            bg.BackgroundColor3 = Color3.fromRGB(18, 16, 23)
            bg.BackgroundTransparency = 0
            bg.ImageTransparency = 1
            ...
            local list = bg:FindFirstChild("ScrollingFrame")
            if list then
                list.Position = UDim2.new(0, 10, 0, 40)
                list.Size = UDim2.new(0, 106, 0, 238)
                list.Visible = true
                list.Active = true
                list.BackgroundColor3 = Color3.fromRGB(13, 12, 17)
                list.BackgroundTransparency = 0
                ...
                local buttonNames = {"Dex", "RemoteSpy", "UnnamedESP", "ScriptDumper", "ShiftLock"}
                for i, name in ipairs(buttonNames) do
                    ...
                    if button and button:IsA("TextButton") then
                        ...
                        button.TextXAlignment = Enum.TextXAlignment.Left
                        button.TextYAlignment = Enum.TextYAlignment.Center
                        local pad = button:FindFirstChild("ReSaveHubButtonPadding")
                        if not pad then
                            pad = Instance.new("UIPadding")
                            pad.Name = "ReSaveHubButtonPadding"
                            pad.Parent = button
                        end
                        pad.PaddingLeft = UDim.new(0, 9)
                        setCorner(button, 5)
        ...
        local panel = hub:FindFirstChild("Panel")
        if panel then
            panel.Position = UDim2.new(0, 1, 0, 1)
            panel.Size = UDim2.new(1, -2, 0, 29)
            panel.ZIndex = 120
            panel.BackgroundColor3 = Color3.fromRGB(20, 16, 27)
            panel.BackgroundTransparency = 0
            panel.BorderSizePixel = 0
            setCorner(panel, 9)
        end

        -- Keep the logo separated from the title.
        local icon = hub:FindFirstChild("Icon")
        if icon then
            icon.Visible = true
            icon.ZIndex = 132
            ...
            icon.Image = "rbxassetid://96720490324068"
        end
        ...
        -- Reuse a single drag bar so the whole popup moves, not just the label.
        local dragBar = hub:FindFirstChild("ReSaveHubDragBar")
        if not dragBar then
            dragBar = Instance.new("Frame")
            dragBar.Name = "ReSaveHubDragBar"
        ...
        dragBar.Position = UDim2.new(0, 36, 0, 0)
        dragBar.Size = UDim2.new(1, -96, 0, 30)
        dragBar.BackgroundTransparency = 1
        dragBar.BorderSizePixel = 0
        dragBar.Active = true
        dragBar.ZIndex = 135
        ...
        if minus then
            ...
            minus.Text = ""
            setCorner(minus, 5)
            local line = minus:FindFirstChild("ReSaveHubMinusLine")
            if not line then
                line = Instance.new("Frame")
                line.Name = "ReSaveHubMinusLine"
                line.Parent = minus
            end
            line.AnchorPoint = Vector2.new(0.5, 0.5)
            line.Position = UDim2.new(0.5, 0, 0.55, 0)
            line.Size = UDim2.new(0, 10, 0, 2)
            line.BackgroundColor3 = Color3.fromRGB(244, 241, 252)
            line.BorderSizePixel = 0
            line.ZIndex = 161
        end
        ...
        if close then
            ...
            close.Draggable = false
            close.ZIndex = 160
            close.Position = UDim2.new(0, 257, 1, -38)
            close.Size = UDim2.new(0, 121, 0, 28)
            close.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
            close.BackgroundTransparency = 0
            close.Text = "Close"
        ...
        if execute then
            execute.Visible = true
            execute.Active = true
            execute.Draggable = false
            execute.ZIndex = 160
            execute.Position = UDim2.new(0, 125, 1, -38)
            execute.Size = UDim2.new(0, 121, 0, 28)
            execute.BackgroundColor3 = Color3.fromRGB(63, 38, 86)
            execute.BackgroundTransparency = 0
        ...
        -- Force the ShiftLock preview once on startup using the same Content API
        -- path as the click handler. Do not use a square thumbnail here.
        local shiftFrame = hub:FindFirstChild("ShiftLock")
        local shiftImage = shiftFrame and shiftFrame:FindFirstChild("Image")
        if shiftFrame and shiftImage and (shiftImage:IsA("ImageLabel") or shiftImage:IsA("ImageButton")) then
            local uri = "rbxassetid://94251318161622"
            ...
            pcall(function()
                shiftImage.ImageContent = Content.fromUri(uri)
                contentOK = true
            end)

            if not contentOK then
                pcall(function()
                    shiftImage.Image = uri
                end)
            end

            task.spawn(function()
                pcall(function()
                    game:GetService("ContentProvider"):PreloadAsync({shiftImage})
                end)
            end)
        end

        -- Rebuild drag handling once, after all the layout changes.
        if not hub:GetAttribute("ReSaveFinalDragBound") then
            hub:SetAttribute("ReSaveFinalDragBound", true)
            local UIS = game:GetService("UserInputService")
            local dragging = false
            local dragInput = nil
            local dragStart = nil
    ...
    -- ---------------- Options ----------------
    local options = root:FindFirstChild("OptionMenu")
    if options then
        options.Visible = false
        options.Active = true
        options.AnchorPoint = Vector2.new(0.5, 0.5)
        options.Position = UDim2.new(0.5, 0, 0.5, 0)
        options.Size = UDim2.new(0, 300, 0, 282)
        options.BackgroundColor3 = Color3.fromRGB(14, 12, 19)
        options.BackgroundTransparency = 0
        options.BorderSizePixel = 0
        options.ClipsDescendants = true
        options.ZIndex = 200
        setCorner(options, 10)
        setStroke(options, Color3.fromRGB(151, 94, 246), 1.2, 0.18)

        local body = options:FindFirstChild("MainFrame")
        if body then
            body.Position = UDim2.new(0, 1, 0, 31)
            body.Size = UDim2.new(1, -2, 1, -32)
            body.BackgroundColor3 = Color3.fromRGB(18, 16, 23)
        ...
        local optionTitle = options:FindFirstChild("Title")
        if optionTitle then
            optionTitle.Position = UDim2.new(0, 40, 0, 3)
            optionTitle.Size = UDim2.new(1, -80, 0, 25)
            optionTitle.ZIndex = 214
            optionTitle.Text = "Options"
            optionTitle.TextSize = 15
            optionTitle.TextColor3 = Color3.fromRGB(244, 241, 252)
            optionTitle.TextXAlignment = Enum.TextXAlignment.Left
            optionTitle.TextYAlignment = Enum.TextYAlignment.Center
            optionTitle.BackgroundTransparency = 1
        end

        local optionIcon = options:FindFirstChild("Icon")
        if optionIcon then
            optionIcon.Position = UDim2.new(0, 8, 0, 3)
            optionIcon.Size = UDim2.new(0, 24, 0, 24)
            optionIcon.ZIndex = 215
            optionIcon.BackgroundTransparency = 1
            optionIcon.ImageTransparency = 0
            optionIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
            optionIcon.ScaleType = Enum.ScaleType.Fit
            optionIcon.Image = "rbxassetid://96720490324068"
        end

        -- The old Close button is retained as the functional top-right '-'.
        local optionClose = options:FindFirstChild("Close")
        if optionClose then
            optionClose.Visible = true
            optionClose.Active = true
            optionClose.Draggable = false
            optionClose.Position = UDim2.new(1, -34, 0, 4)
            optionClose.Size = UDim2.new(0, 28, 0, 24)
            optionClose.ZIndex = 220
            optionClose.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
            optionClose.BackgroundTransparency = 0
            optionClose.BorderSizePixel = 0
            optionClose.Text = ""
            setCorner(optionClose, 5)
            local line = optionClose:FindFirstChild("ReSaveOptionMinusLine")
            if not line then
                line = Instance.new("Frame")
                line.Name = "ReSaveOptionMinusLine"
                line.Parent = optionClose
            end
            line.AnchorPoint = Vector2.new(0.5, 0.5)
            line.Position = UDim2.new(0.5, 0, 0.58, 0)
            line.Size = UDim2.new(0, 11, 0, 2)
            line.BackgroundColor3 = Color3.fromRGB(244, 241, 252)
            line.BorderSizePixel = 0
            line.ZIndex = 221
        end
        ...
        local labels = {
            {"FPSUnlocker", "FPS Unlock [BETA]", 52},
            {"AutoExec", "Auto Exec scripts", 88},
            {"InternalUI", "Internal UI", 124},
            {"TopMost", "TopMost", 160},
        }
        for _, item in ipairs(labels) do
            local name, textValue, y = item[1], item[2], item[3]
            local label = options:FindFirstChild(name)
            if label then
                label.Visible = true
                label.ZIndex = 212
        ...
        local toggles = {
            {"ToggleFPSUnlocker", 52},
            {"ToggleAutoExec", 88},
            {"ToggleInternalUI", 124},
            {"ToggleTopMost", 160},
        }
        for _, item in ipairs(toggles) do
            local name, y = item[1], item[2]
            local toggle = options:FindFirstChild(name)
            if toggle then
                toggle.Visible = true
                toggle.Active = true
                toggle.Draggable = false
                toggle.ZIndex = 215
                toggle.Position = UDim2.new(1, -42, 0, y + 4)
                toggle.Size = UDim2.new(0, 20, 0, 20)
                toggle.BackgroundTransparency = 0
                toggle.BorderSizePixel = 0
                toggle.TextColor3 = Color3.fromRGB(245, 243, 255)
                toggle.TextSize = 13
                setCorner(toggle, 5)
            end
        end

        local buttons = options:FindFirstChild("Buttons")
        if buttons then
            buttons.Position = UDim2.new(0, 18, 0, 199)
            buttons.Size = UDim2.new(1, -36, 0, 62)
            buttons.ZIndex = 210
            ...
            local rejoin = buttons:FindFirstChild("Rejoin")
            if rejoin then
                rejoin.Position = UDim2.new(0, 0, 0, 0)
                rejoin.Size = UDim2.new(0.5, -3, 0, 26)
                rejoin.ZIndex = 215
                rejoin.BackgroundColor3 = Color3.fromRGB(27, 22, 34)
                rejoin.BackgroundTransparency = 0
                rejoin.BorderSizePixel = 0
                rejoin.TextColor3 = Color3.fromRGB(244, 241, 252)
                rejoin.TextSize = 13
                setCorner(rejoin, 6)
            end

            local discord = buttons:FindFirstChild("Discord")
            if discord then
                discord.Position = UDim2.new(0.5, 3, 0, 0)
                discord.Size = UDim2.new(0.5, -3, 0, 26)
                discord.ZIndex = 215
                discord.BackgroundColor3 = Color3.fromRGB(27, 22, 34)
                discord.BackgroundTransparency = 0
                discord.BorderSizePixel = 0
                discord.TextColor3 = Color3.fromRGB(244, 241, 252)
                discord.TextSize = 13
                setCorner(discord, 6)
            end
        end

        -- Keep the option handler functional; only its visual layout changes.
        if not options:GetAttribute("ReSaveOptionsCloseVisibleBound") then
            options:SetAttribute("ReSaveOptionsCloseVisibleBound", true)
            local closeButton = options:FindFirstChild("Close")
            if closeButton then
    ...
    -- Make the Options toolbar button place the popup at the repaired center
    -- position rather than the old generated off-screen coordinates.
    local main = root:FindFirstChild("Main")
    local optionsButton = main and main:FindFirstChild("Options")
    if optionsButton and options then
        if not optionsButton:GetAttribute("ReSaveOptionsOpenBound") then
            optionsButton:SetAttribute("ReSaveOptionsOpenBound", true)
            optionsButton.MouseButton1Up:Connect(function()
                options.AnchorPoint = Vector2.new(0.5, 0.5)
                options.Position = UDim2.new(0.5, 0, 0.5, 0)
...
-- ============================================================
-- ReSave ShiftLock preview repair.
-- Uses Roblox's Content/ImageContent API and preserves the original
-- 512:277 aspect ratio so the horizontal preview is never stretched.
-- ============================================================
do
    local root = G2L["1"]
    local hub = root and root:FindFirstChild("ScriptHubMenu")
    if hub then
        local shiftFrame = hub:FindFirstChild("ShiftLock")
        local shiftImage = shiftFrame and shiftFrame:FindFirstChild("Image")

        if shiftFrame and shiftImage and (shiftImage:IsA("ImageLabel") or shiftImage:IsA("ImageButton")) then
            local SHIFTLOCK_ID = "94251318161622"
            local SHIFTLOCK_URI = "rbxassetid://" .. SHIFTLOCK_ID

            -- The source image is 512x277 (1.848:1). Give the preview
            -- container the same ratio instead of squeezing it into 275x126.
            shiftFrame.Visible = false
            shiftFrame.Position = UDim2.new(0, 125, 0, 48)
            shiftFrame.Size = UDim2.new(0, 275, 0, 149)
            shiftFrame.BackgroundTransparency = 1
            shiftFrame.ZIndex = 140

            local aspect = shiftImage:FindFirstChild("ReSaveShiftLockAspect")
            if not aspect then
                aspect = Instance.new("UIAspectRatioConstraint")
                aspect.Name = "ReSaveShiftLockAspect"
                aspect.Parent = shiftImage
            end
            ...
            shiftImage.Position = UDim2.new(0, 0, 0, 0)
            shiftImage.Size = UDim2.new(1, 0, 1, 0)
            shiftImage.AnchorPoint = Vector2.new(0, 0)
            shiftImage.BackgroundTransparency = 1
            shiftImage.ImageTransparency = 0
            shiftImage.ImageColor3 = Color3.new(1, 1, 1)
            shiftImage.ScaleType = Enum.ScaleType.Fit
            shiftImage.ZIndex = 141

            -- Use the Content API rather than the legacy HTTP asset string.
            -- ImageContent is the modern content property for ImageLabel.
            local contentAssigned = false
            pcall(function()
                shiftImage.ImageContent = Content.fromUri(SHIFTLOCK_URI)
                contentAssigned = true
            end)

            -- Keep Image populated as a compatibility path for older Studio builds.
            pcall(function()
                shiftImage.Image = SHIFTLOCK_URI
            end)

            -- If the content API is available, preload through the actual GUI object.
            task.spawn(function()
                local contentProvider = game:GetService("ContentProvider")
                pcall(function()
                    contentProvider:PreloadAsync({shiftImage})
                end)

                -- If ImageContent was accepted but still has not loaded, retry the URI
                -- assignment once without introducing square thumbnail dimensions.
                if not shiftImage.IsLoaded then
                    pcall(function()
                        shiftImage.ImageContent = Content.fromUri(SHIFTLOCK_URI)
                    end)
                    pcall(function()
        ...
        -- Match the larger horizontal preview with the lower information panel.
        -- This only affects the Script Hub's visual layout.
        hub.Size = UDim2.new(0, 420, 0, 310)
        ...
        local desc = hub:FindFirstChild("Description")
        if desc then
            desc.Position = UDim2.new(0, 125, 0, 204)
            desc.Size = UDim2.new(0, 275, 0, 42)
            desc.Visible = true
            desc.ZIndex = 150
        end
...
-- ============================================================
-- ReSave v2.26.upg2.4 — final UI state repair pass
-- This block intentionally runs LAST so legacy/generated layout code cannot
-- move the action buttons back to the old positions or hide the Log button.
-- ============================================================
do
    local root = G2L["1"]
    ...
    if root and main then
        ...
        local function ensureCorner(obj, radius)
            if not obj then return end
            local c = obj:FindFirstChild("ReSaveUpg24Corner")
            if not c then
                c = Instance.new("UICorner")
                c.Name = "ReSaveUpg24Corner"
                c.Parent = obj
            end
            c.CornerRadius = UDim.new(0, radius)
        end

        local function ensureStroke(obj, name, color, thickness, transparency)
            if not obj then return nil end
            local st = obj:FindFirstChild(name)
            if not st then
        ...
        -- ----- Action bar -----
        local toolbar = main:FindFirstChild("ReSaveToolbar")
        if toolbar and toolbar:IsA("ScrollingFrame") then
            toolbar.Position = UDim2.new(0, 1, 0, 241)
            toolbar.Size = UDim2.new(1, -2, 0, 49)
            ...
            toolbar.ScrollingDirection = Enum.ScrollingDirection.X
            toolbar.ScrollBarThickness = 4
            toolbar.ScrollBarImageColor3 = Color3.fromRGB(151, 94, 246)
            toolbar.ScrollBarImageTransparency = 0.20
            toolbar.AutomaticCanvasSize = Enum.AutomaticSize.None
            toolbar.CanvasPosition = Vector2.new(0, 0)
            pcall(function()
                toolbar.HorizontalScrollBarInset = Enum.ScrollBarInset.Always
            end)
            ensureCorner(toolbar, 7)
            ...
            content.BorderSizePixel = 0
            content.Position = UDim2.new(0, 6, 0, 2)
            content.Size = UDim2.new(0, 0, 0, 32)
            content.ZIndex = 51

            local names = {"Execute","Clear","OpenFile","SaveFile","Options","Attach","ScriptHub","Log"}
            local width, gap, height = 82, 7, 30
            local total = (#names * width) + ((#names - 1) * gap)
            content.Size = UDim2.new(0, total, 0, 32)
            toolbar.CanvasSize = UDim2.new(0, total + 12, 0, 32)

            local function findActionButton(name)
                local inContent = content:FindFirstChild(name)
                if inContent and inContent:IsA("GuiButton") then
                    return inContent
                end

                local direct = main:FindFirstChild(name)
            ...
            for index, name in ipairs(names) do
                local button = findActionButton(name)
                if button and button:IsA("GuiButton") then
                    -- Re-parent after ALL old repair passes. This is the key part that
                    -- prevents later Position assignments from being interpreted in Main.
                    if button.Parent ~= content then
                        button.Parent = content
                    end
                    ...
                    button.AutoButtonColor = false
                    button.Position = UDim2.new(0, (index - 1) * (width + gap), 0, 0)
                    button.Size = UDim2.new(0, width, 0, height)
                    button.ZIndex = 60
                    button.BorderSizePixel = 0
                    ensureCorner(button, 7)

                    local label = button:FindFirstChild("NameText")
                    if label and label:IsA("TextLabel") then
                        label.Visible = true
                        label.TextColor3 = Color3.fromRGB(245, 242, 250)
                        label.TextSize = 13
                        label.ZIndex = 62
                    end
                end
            ...
            -- Log button: explicit purple edge + white text, matching the other actions.
            local logButton = findActionButton("Log")
            if logButton then
                logButton.Parent = content
                logButton.Visible = true
                logButton.Text = "Log"
                logButton.TextColor3 = Color3.fromRGB(245, 242, 250)
                logButton.TextTransparency = 0
                logButton.BackgroundColor3 = Color3.fromRGB(27, 22, 34)
                logButton.BackgroundTransparency = 0
                logButton.BorderSizePixel = 1
                logButton.BorderColor3 = Color3.fromRGB(151, 94, 246)
                ensureStroke(logButton, "ReSaveLogPurpleStroke", Color3.fromRGB(151, 94, 246), 1.15, 0.05)
                ensureCorner(logButton, 7)
            end

            -- Keep the scrollbar visually separated from the clickable buttons.
            local barGap = toolbar:FindFirstChild("ReSaveActionBarGap")
            if not barGap then
                barGap = Instance.new("Frame")
                barGap.Name = "ReSaveActionBarGap"
                barGap.Parent = toolbar
            end
            barGap.Position = UDim2.new(0, 5, 0, 33)
            barGap.Size = UDim2.new(0, total + 2, 0, 1)
            barGap.BackgroundColor3 = Color3.fromRGB(42, 34, 49)
            barGap.BackgroundTransparency = 0.45
            barGap.BorderSizePixel = 0
            barGap.ZIndex = 55

            -- Make the native scrollbar's canvas area independent from the buttons.
            pcall(function()
                toolbar.VerticalScrollBarInset = Enum.ScrollBarInset.None
            end)
        end
        ...
        if logConsole then
            logConsole.ZIndex = 800
            logConsole.Active = true
            logConsole.Draggable = false

            local header = logConsole:FindFirstChild("Header")
            if header then
                header.Active = true
            ...
            if not logConsole:GetAttribute("ReSaveLogDragBound24") and header then
                logConsole:SetAttribute("ReSaveLogDragBound24", true)
        ...
        -- ----- Attach state: bright border while waiting, normal border after attach -----
        local attachButton = nil
        local actionBar = main:FindFirstChild("ReSaveToolbar")
        local actionContent = actionBar and actionBar:FindFirstChild("ActionContent")
        if actionContent then
            attachButton = actionContent:FindFirstChild("Attach")
        end
        if not attachButton then
            attachButton = main:FindFirstChild("Attach")
        end
        if attachButton and attachButton:IsA("GuiButton") then
            local normal = Color3.fromRGB(87, 59, 108)
            local waiting = Color3.fromRGB(151, 94, 246)
            local attachedStroke = ensureStroke(
                attachButton,
                "ReSaveAttachStateStroke",
                normal,
                1.25,
                0.05
            )

            local function attachedState()
                local injected = root:FindFirstChild("Injected")
                local value = false
                if injected and injected:IsA("BoolValue") then
                    value = injected.Value == true
                end
                return value or root:GetAttribute("ReSaveAttached") == true
            end

            local function updateAttachVisual()
                local isAttached = attachedState()
                if attachedStroke and attachedStroke.Parent then
                    attachedStroke.Color = isAttached and normal or waiting
                    attachedStroke.Thickness = isAttached and 1.25 or 1.9
                    attachedStroke.Transparency = isAttached and 0.12 or 0
                end
                attachButton.BorderSizePixel = 1
                attachButton.BorderColor3 = isAttached and normal or waiting
                attachButton:SetAttribute("ReSaveAttachHighlightActive", not isAttached)
            end

            updateAttachVisual()

            local injected = root:FindFirstChild("Injected")
            if injected and injected:IsA("BoolValue") and not root:GetAttribute("ReSaveAttachStateBound24") then
                root:SetAttribute("ReSaveAttachStateBound24", true)
                injected:GetPropertyChangedSignal("Value"):Connect(updateAttachVisual)
                root:GetAttributeChangedSignal("ReSaveAttached"):Connect(updateAttachVisual)
            end
        end

        -- Record the completed upgrade once, not every time a render callback fires.
        if not root:GetAttribute("ReSaveUpg24ReadyLogged") then
            root:SetAttribute("ReSaveUpg24ReadyLogged", true)
            ReSaveLog(
                "INFO",
                "UPGRADE",
                "ReSave v2.26.upg2.4 UI repair loaded",
                "Log=purple-bordered\nActionBar=horizontal-scroll-separated\nClear=animated+logged\nLogDrag=enabled\nAttachState=waiting/attached"
            )
        end
    end
end
