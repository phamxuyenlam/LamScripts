...
local function C_71()
	...
	local list = hub.Background.ScrollingFrame

	local descriptions = {
		Dex = [[A version of the popular Dex explorer with
patches specifically for ReSave.]],
		RemoteSpy = [[Allows you to view RemoteEvents and
RemoteFunctions called.]],
		UnnamedESP = [[ESP made by ic3w0lf using the Drawing API.]],
		ScriptDumper = [[Dumps all LocalScripts and ModuleScripts.]],
		ShiftLock = [[A lightweight ShiftLock utility from LamScripts.]],
	}

	local function selectScript(name)
		for _, child in ipairs(hub:GetChildren()) do
			if child:IsA("Frame") and child ~= hub and child ~= hub.Panel and child.Name ~= "ReSaveHubDragBar" then
				local keep = child.Name == name
				child.Visible = keep
			end
		end

		-- Never let the title/header surface get hidden by selection logic.
		...
		hub.Description.Text = descriptions[name] or ""
		hub.Description.Visible = true

		local selected = hub:FindFirstChild(name)
		if selected then
	...
	hub.Execute.MouseButton1Up:Connect(function()
		...
		if selectedName == "Dex" then
			loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/main/dex.lua"))()
		elseif selectedName == "RemoteSpy" then
			loadstring(game:HttpGet("https://raw.githubusercontent.com/infyiff/backup/main/SimpleSpyV3/main.lua"))()
		elseif selectedName == "UnnamedESP" then
			pcall(function()
				loadstring(game:HttpGet("https://raw.githubusercontent.com/ic3w0lf22/Unnamed-ESP/master/UnnamedESP.lua"))()
			end)
		elseif selectedName == "ScriptDumper" then
			saveinstance()
		elseif selectedName == "ShiftLock" then
	...
	-- Start with Dex selected so the preview/description/action buttons are populated.
	selectScript("Dex")
end;
task.spawn(C_71);
-- StarterGui.ReSave.OptionMenu.Optionhandler
