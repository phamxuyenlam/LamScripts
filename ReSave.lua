...
local resaveversion = "2.26.upg2.2"
...
local function ReSaveLog(level, eventName, message, details)
    ...
    local entry = {
        ...
        event = tostring(eventName or "APP"),
...
do
	...
	-- Part 2: emphasize Attach while ReSave is still unattached.
	local attachButton = main and main:FindFirstChild("Attach")
	if attachButton then
		root:SetAttribute("ReSaveAttached", root.Injected.Value == true)
		local attachStroke = attachButton:FindFirstChild("ReSaveAttachPulse")
		if not attachStroke then
			attachStroke = Instance.new("UIStroke")
			attachStroke.Name = "ReSaveAttachPulse"
			attachStroke.Thickness = 1.7
			attachStroke.Parent = attachButton
		end
		attachStroke.Color = C.purple2
		attachStroke.Transparency = root:GetAttribute("ReSaveAttached") and 0.35 or 0.02
		attachButton.ZIndex = 70
		attachButton.AutoButtonColor = false

		task.spawn(function()
			local tweenInfo = TweenInfo.new(0.65, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
			while root.Parent and attachStroke.Parent do
				if root:GetAttribute("ReSaveAttached") == true then
					attachStroke.Transparency = 0.45
					attachButton.BackgroundColor3 = C.purpleDark
					break
				end

				TweenService:Create(attachStroke, tweenInfo, {Transparency = 0.45, Thickness = 2.2}):Play()
				TweenService:Create(attachButton, tweenInfo, {BackgroundColor3 = Color3.fromRGB(58, 34, 79)}):Play()
				task.wait(0.65)
				if not root.Parent then break end
				TweenService:Create(attachStroke, tweenInfo, {Transparency = 0.02, Thickness = 1.7}):Play()
				TweenService:Create(attachButton, tweenInfo, {BackgroundColor3 = C.purpleDark}):Play()
				task.wait(0.65)
			end
		end)
	end
...
-- ============================================================
-- ReSave v2.26.upg2 — Log Console + Attach state repair
-- ============================================================
do
    local root = G2L["1"]
    local main = root and root:FindFirstChild("Main")
    local console = root and root:FindFirstChild("ScriptLog")

    local LEVEL_COLORS = {
        ERROR = Color3.fromRGB(255, 115, 115),
        WARN = Color3.fromRGB(255, 206, 112),
        INFO = Color3.fromRGB(214, 198, 255),
        OUTPUT = Color3.fromRGB(190, 190, 205),
    }

    local function setCornerSafe(obj, radius)
        if not obj then return end
        local corner = obj:FindFirstChild("ReSaveUpg2Corner")
        if not corner then
            corner = Instance.new("UICorner")
            corner.Name = "ReSaveUpg2Corner"
            corner.Parent = obj
        end
        corner.CornerRadius = UDim.new(0, radius)
    end

    local function setStrokeSafe(obj, color, thickness, transparency)
        if not obj then return end
        local stroke = obj:FindFirstChild("ReSaveUpg2Stroke")
        if not stroke then
            stroke = Instance.new("UIStroke")
            stroke.Name = "ReSaveUpg2Stroke"
            stroke.Parent = obj
        end
        stroke.Color = color
        stroke.Thickness = thickness
    ...
    if main then
        -- A compact top-right Log button sits beside the window controls.
        ...
        logButton.Active = true
        logButton.AutoButtonColor = false
        logButton.Text = "Log"
        logButton.Font = Enum.Font.GothamMedium
        logButton.TextSize = 12
        logButton.TextColor3 = Color3.fromRGB(244, 241, 252)
        logButton.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
        logButton.BackgroundTransparency = 0
        logButton.BorderSizePixel = 0
        logButton.Position = UDim2.new(0, 527, 0, 3)
        logButton.Size = UDim2.new(0, 48, 0, 22)
        logButton.ZIndex = 125
        setCornerSafe(logButton, 5)
        setStrokeSafe(logButton, Color3.fromRGB(82, 51, 112), 1, 0.05)

        local logLabel = logButton:FindFirstChild("NameText")
        if logLabel then
            logLabel.Visible = false
        end

        if not logButton:GetAttribute("ReSaveUpg2Bound") then
            logButton:SetAttribute("ReSaveUpg2Bound", true)
            logButton.Activated:Connect(function()
                if console then
                    console.Visible = not console.Visible
                    ReSaveLog("INFO", "LOG CONSOLE", console.Visible and "Log Console opened" or "Log Console closed")
                end
            end)
        end
    end
    ...
    if console then
        ...
        console.Size = UDim2.new(0, 470, 0, 315)
        console.BackgroundColor3 = Color3.fromRGB(13, 11, 18)
        console.BackgroundTransparency = 0
        console.BorderSizePixel = 0
        console.ZIndex = 300
        console.ClipsDescendants = true
        setCornerSafe(console, 10)
        setStrokeSafe(console, Color3.fromRGB(151, 94, 246), 1.2, 0.12)

        local header = console:FindFirstChild("Upg2Header")
        if not header then
            header = Instance.new("Frame")
            header.Name = "Upg2Header"
        ...
        local titleLabel = console:FindFirstChild("Title")
        if titleLabel then
            ...
            titleLabel.TextColor3 = Color3.fromRGB(244, 241, 252)
            titleLabel.BackgroundTransparency = 1
            titleLabel.ZIndex = 303
        end

        local icon = console:FindFirstChild("Icon")
        if icon then
            icon.Visible = true
            icon.Position = UDim2.new(1, -83, 0, 7)
            icon.Size = UDim2.new(0, 20, 0, 20)
            icon.ZIndex = 303
        end

        local oldList = console:FindFirstChild("LogList")
        if not oldList then
            oldList = console:FindFirstChild("MainFrame") and console.MainFrame:FindFirstChild("ANS9DZNASD8Z7NAS987NAFA")
            if oldList then oldList.Name = "LogList" end
        end

        if oldList then
            if oldList.Parent ~= console then
                oldList.Parent = console
            end
            oldList.Visible = true
            oldList.Active = true
            oldList.Position = UDim2.new(0, 10, 0, 43)
            oldList.Size = UDim2.new(1, -20, 1, -92)
            oldList.BackgroundColor3 = Color3.fromRGB(8, 8, 12)
            oldList.BackgroundTransparency = 0
            oldList.BorderSizePixel = 0
            oldList.ScrollBarThickness = 5
            oldList.CanvasSize = UDim2.new(0, 0, 0, 0)
            oldList.AutomaticCanvasSize = Enum.AutomaticSize.Y
            oldList.ZIndex = 302
            setCornerSafe(oldList, 7)
            ...
            local layout = oldList:FindFirstChildOfClass("UIListLayout")
            if not layout then
                layout = Instance.new("UIListLayout")
                layout.Parent = oldList
            end
            layout.SortOrder = Enum.SortOrder.LayoutOrder
            layout.Padding = UDim.new(0, 4)

            local padding = oldList:FindFirstChildOfClass("UIPadding")
            if not padding then
                padding = Instance.new("UIPadding")
                padding.Parent = oldList
            end
            padding.PaddingTop = UDim.new(0, 6)
            padding.PaddingBottom = UDim.new(0, 6)
            ...
            -- This is the only renderer used by ReSaveLog.
            ReSaveLogRender = function()
                if not oldList or not oldList.Parent then return end

                local existingIds = {}
                for _, child in ipairs(oldList:GetChildren()) do
                    if child:IsA("TextLabel") and child:GetAttribute("ReSaveLogRow") == true then
                        existingIds[child:GetAttribute("LogId")] = child
                    end
                end
                ...
                for _, entry in ipairs(ReSaveLogEntries) do
                    if not existingIds[entry.id] then
                        ...
                        row.Name = "Log_" .. tostring(entry.id)
                        row:SetAttribute("ReSaveLogRow", true)
                        row:SetAttribute("LogId", entry.id)
                        ...
                        row.TextSize = 12
                        row.TextColor3 = LEVEL_COLORS[entry.level] or LEVEL_COLORS.INFO
                        row.ZIndex = 304

                        local text = string.format("[%s] [%s] %s | %s", entry.time, entry.level, entry.event, entry.message)
                ...
                for id, row in pairs(existingIds) do
                    ...
                    if not stillThere and row and row.Parent then
                        row:Destroy()
                    end
                end

                task.defer(function()
                    if oldList and oldList.Parent then
                        oldList.CanvasPosition = Vector2.new(0, math.max(0, oldList.AbsoluteCanvasSize.Y - oldList.AbsoluteSize.Y))
                    end
                end)
            end
        end

        local clearButton = console:FindFirstChild("ClearLogs")
        ...
        clearButton.TextColor3 = Color3.fromRGB(244, 241, 252)
        clearButton.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
        clearButton.BorderSizePixel = 0
        clearButton.Position = UDim2.new(1, -128, 0, 5)
        clearButton.Size = UDim2.new(0, 58, 0, 24)
        clearButton.ZIndex = 304
        setCornerSafe(clearButton, 5)

        local closeButton = console:FindFirstChild("Close")
        if not closeButton then
            closeButton = console:FindFirstChild("MainFrame") and console.MainFrame:FindFirstChild("A8SDMZAS89DZANSA98F")
            if closeButton then closeButton.Name = "Close" end
        end
        if closeButton then
            if closeButton.Parent ~= console then
                closeButton.Parent = console
            end
            closeButton.Active = true
            closeButton.Text = "×"
            closeButton.Font = Enum.Font.GothamSemibold
            closeButton.TextSize = 16
            closeButton.TextColor3 = Color3.fromRGB(244, 241, 252)
            closeButton.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
            closeButton.BackgroundTransparency = 0
            closeButton.BorderSizePixel = 0
            closeButton.Position = UDim2.new(1, -40, 0, 5)
            closeButton.Size = UDim2.new(0, 28, 0, 24)
            closeButton.ZIndex = 304
            setCornerSafe(closeButton, 5)

            if not closeButton:GetAttribute("ReSaveUpg2Bound") then
                closeButton:SetAttribute("ReSaveUpg2Bound", true)
        ...
        if not clearButton:GetAttribute("ReSaveUpg2Bound") then
            clearButton:SetAttribute("ReSaveUpg2Bound", true)
        ...
        -- Hide legacy ScriptLog controls that are no longer part of the console.
        if console:FindFirstChild("MainFrame") then
            local legacyTextBox = console.MainFrame:FindFirstChild("9NAC7A9S7N8ZASFH9ASF87NAS8YGA9GSA7")
            if legacyTextBox then legacyTextBox.Visible = false end
            local legacyCopy = console.MainFrame:FindFirstChild("9A8D7NAS9Z87NZDA98S7DNA98DNZ9A8SN")
            if legacyCopy then legacyCopy.Visible = false end
            local legacyClose = console.MainFrame:FindFirstChild("A8SDMZAS89DZANSA98F")
            if legacyClose and legacyClose ~= closeButton then legacyClose.Visible = false end
            local legacyScript = console.MainFrame:FindFirstChild("ANS9DZNASD8Z7NAS987NAFA")
            if legacyScript and legacyScript ~= oldList then legacyScript.Visible = false end
            console.MainFrame.BackgroundTransparency = 1
        end

        -- Render all buffered startup messages once the console UI exists.
        if ReSaveLogRender then
            pcall(ReSaveLogRender)
        end
    end

    -- Fix the previous Attach highlight bug: once attached, remove the pulse
    -- stroke instead of leaving a permanent outline behind.
    local function clearAttachHighlight()
        ...
        if root.Injected.Value == true or root:GetAttribute("ReSaveAttached") == true then
            if pulse then
                pulse:Destroy()
            end
    ...
    if root.Injected then
        root.Injected:GetPropertyChangedSignal("Value"):Connect(function()
            if root.Injected.Value == true then
                ReSaveLog("INFO", "ATTACH", "Attached state confirmed by Injected.Value", "highlight=off")
                clearAttachHighlight()
            end
        end)
    end
    ...
    ReSaveLog("INFO", "STARTUP", "ReSave v2.26.upg2 initialized", "Log Console=ready\nAttach highlight=auto-clear\nmaxLogEntries=" .. tostring(ReSaveLogMaxEntries))
end

-- ============================================================
-- ReSave v2.26.upg2.2 — hard repair for Log Console + Attach
-- ============================================================
do
    local root = G2L["1"]
    local main = root and root:FindFirstChild("Main")
    ...
    local function corner(obj, radius, name)
        if not obj then
            return
        end
    ...
    -- Log button: direct child of Main, high ZIndex, guaranteed visible.
    local logButton
    if main then
        logButton = main:FindFirstChild("Log")

        if not logButton or not logButton:IsA("TextButton") then
            destroySafe(logButton)
        ...
        logButton.BackgroundColor3 = Color3.fromRGB(31, 24, 42)
        logButton.BackgroundTransparency = 0
        logButton.BorderSizePixel = 0
        logButton.Position = UDim2.new(0, 573, 0, 2)
        logButton.Size = UDim2.new(0, 48, 0, 22)
        logButton.ZIndex = 10000
        corner(logButton, 5, "ReSaveUpg22Corner")

        -- Disable any old child overlay that could hide the text.
        for _, child in ipairs(logButton:GetChildren()) do
    ...
    -- Fresh console. This does not reuse the old ScriptLog layout.
    local console = root:FindFirstChild("ReSaveLogConsole22")
    if not console then
        console = Instance.new("Frame")
        console.Name = "ReSaveLogConsole22"
        console.Parent = root
    end
    ...
    console.AnchorPoint = Vector2.new(0.5, 0.5)
    console.Position = UDim2.new(0.5, 0, 0.5, 0)
    console.Size = UDim2.new(0, 510, 0, 350)
    console.BackgroundColor3 = Color3.fromRGB(10, 9, 14)
    console.BackgroundTransparency = 0
    console.BorderSizePixel = 0
    console.ZIndex = 12000
    console.ClipsDescendants = true
    corner(console, 10, "Corner")

    local header = console:FindFirstChild("Header")
    if not header then
        header = Instance.new("Frame")
        header.Name = "Header"
        header.Parent = console
    ...
    header.BorderSizePixel = 0
    header.Position = UDim2.new(0, 1, 0, 1)
    header.Size = UDim2.new(1, -2, 0, 38)
    header.ZIndex = 12001
    ...
    titleLabel.BackgroundTransparency = 1
    titleLabel.Position = UDim2.new(0, 13, 0, 5)
    titleLabel.Size = UDim2.new(1, -150, 0, 28)
    titleLabel.Font = Enum.Font.GothamSemibold
    titleLabel.Text = "Log Console"
    titleLabel.TextColor3 = Color3.fromRGB(245, 242, 250)
    titleLabel.TextSize = 15
    titleLabel.TextXAlignment = Enum.TextXAlignment.Left
    titleLabel.TextYAlignment = Enum.TextYAlignment.Center
    titleLabel.ZIndex = 12002

    local clearButton = header:FindFirstChild("Clear")
    if not clearButton then
        clearButton = Instance.new("TextButton")
    ...
    clearButton.Font = Enum.Font.GothamMedium
    clearButton.TextSize = 12
    clearButton.TextColor3 = Color3.fromRGB(245, 242, 250)
    clearButton.BackgroundColor3 = Color3.fromRGB(31, 24, 42)
    clearButton.BorderSizePixel = 0
    clearButton.Position = UDim2.new(1, -106, 0, 6)
    clearButton.Size = UDim2.new(0, 58, 0, 25)
    clearButton.ZIndex = 12002
    corner(clearButton, 5, "Corner")

    local closeButton = header:FindFirstChild("Close")
    if not closeButton then
    ...
    closeButton.TextColor3 = Color3.fromRGB(245, 242, 250)
    closeButton.BackgroundColor3 = Color3.fromRGB(31, 24, 42)
    closeButton.BorderSizePixel = 0
    closeButton.Position = UDim2.new(1, -42, 0, 6)
    closeButton.Size = UDim2.new(0, 29, 0, 25)
    closeButton.ZIndex = 12002
    corner(closeButton, 5, "Corner")

    local list = console:FindFirstChild("LogList")
    if not list then
    ...
    list.Visible = true
    list.Active = true
    list.BackgroundColor3 = Color3.fromRGB(7, 7, 10)
    list.BorderSizePixel = 0
    list.Position = UDim2.new(0, 10, 0, 47)
    list.Size = UDim2.new(1, -20, 1, -57)
    list.ScrollBarThickness = 5
    list.CanvasSize = UDim2.new(0, 0, 0, 0)
    list.AutomaticCanvasSize = Enum.AutomaticSize.Y
    list.ScrollingDirection = Enum.ScrollingDirection.Y
    list.ZIndex = 12001
    corner(list, 7, "Corner")

    local layout = list:FindFirstChild("Layout")
    if not layout then
        layout = Instance.new("UIListLayout")
        layout.Name = "Layout"
        layout.Parent = list
    ...
    local pad = list:FindFirstChild("Padding")
    if not pad then
        pad = Instance.new("UIPadding")
        pad.Name = "Padding"
        pad.Parent = list
    ...
    local logColors = {
        ...
        WARN = Color3.fromRGB(255, 208, 120),
        INFO = Color3.fromRGB(216, 201, 255),
        OUTPUT = Color3.fromRGB(190, 190, 205),
    }
    ...
    local function renderLogs()
        ...
        for _, entry in ipairs(ReSaveLogEntries) do
            ...
            row.TextSize = 12
            row.TextColor3 = logColors[entry.level] or logColors.INFO
            row.ZIndex = 12003
        ...
        task.defer(function()
            if list.Parent then
                list.CanvasPosition = Vector2.new(
                    0,
                    math.max(0, list.AbsoluteCanvasSize.Y - list.AbsoluteSize.Y)
                )
            end
        end)
    end

    ReSaveLogRender = renderLogs

    if logButton and not logButton:GetAttribute("ReSaveUpg22Bound") then
        logButton:SetAttribute("ReSaveUpg22Bound", true)
        ...
        logButton.Activated:Connect(function()
            ...
            ReSaveLog(
                "INFO",
                "LOG CONSOLE",
                console.Visible and "Log Console opened" or "Log Console closed"
            )
            renderLogs()
        end)
    end

    if clearButton and not clearButton:GetAttribute("ReSaveUpg22ClearBound") then
        clearButton:SetAttribute("ReSaveUpg22ClearBound", true)
    ...
    if closeButton and not closeButton:GetAttribute("ReSaveUpg22CloseBound") then
        closeButton:SetAttribute("ReSaveUpg22CloseBound", true)
    ...
    -- Attach highlight: hard state binding.
    local attachButton = main and main:FindFirstChild("Attach")
    if attachButton then
        local normalColor = Color3.fromRGB(27, 22, 34)
        local attentionColor = Color3.fromRGB(58, 34, 79)

        local function attached()
            local injected = false
            local injectedObj = root:FindFirstChild("Injected")

            if injectedObj and injectedObj:IsA("BoolValue") then
                injected = injectedObj.Value == true
            end

            return injected or root:GetAttribute("ReSaveAttached") == true
        end

        local function stopHighlight()
            local stroke = attachButton:FindFirstChild("ReSaveAttachPulse")
            destroySafe(stroke)

            attachButton.BackgroundColor3 = normalColor
            attachButton:SetAttribute("ReSaveAttachHighlightActive", false)
        end
        ...
        local function startHighlight()
            ...
            if not stroke then
                stroke = Instance.new("UIStroke")
                stroke.Name = "ReSaveAttachPulse"
                stroke.Parent = attachButton
            end
            ...
            attachButton.BackgroundColor3 = attentionColor
            attachButton:SetAttribute("ReSaveAttachHighlightActive", true)

            task.spawn(function()
                while attachButton.Parent and not attached() do
        ...
        local injectedObj = root:FindFirstChild("Injected")
        if injectedObj and injectedObj:IsA("BoolValue") then
            injectedObj:GetPropertyChangedSignal("Value"):Connect(function()
                if injectedObj.Value then
                    stopHighlight()
                    ReSaveLog(
                        "INFO",
                        "ATTACH",
                        "Attach state confirmed; highlight removed",
                        "highlight=false"
                    )
                else
                    startHighlight()
        ...
        -- Small watchdog for state changes made by another UI handler.
        task.spawn(function()
            while root.Parent and attachButton.Parent do
                task.wait(0.1)
    ...
    ReSaveLog(
        "INFO",
        "STARTUP",
        "ReSave v2.26.upg2.2 initialized",
