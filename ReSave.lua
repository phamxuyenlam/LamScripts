...
local script = G2L["44"];
	local function AddTab()
		...
		-- Mobile-safe editor focus. The old global UserInputService handlers
		-- repeatedly attached InputBegan listeners and competed with the editor.
		textBox.Active = true
		...
		textBox.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.Touch
				...
				task.defer(function()
					if textBox.Parent and not textBox:IsFocused() then
		...
		textBox.Focused:Connect(function()
			-- Keep the line indicator aligned when mobile/desktop focus begins.
			local cursor = math.max(1, textBox.CursorPosition)
			local before = string.sub(textBox.Text, 1, cursor - 1)
			local lineNumber = select(2, before:gsub("\n", "\n")) + 1
			local highlighted = NewTextbox.Frame:FindFirstChild("Highlighted")
			if highlighted then
				highlighted.Position = UDim2.new(0, 0, 0, (lineNumber - 1) * 14)
			end
		end)
...
do
	...
	local function applyLogo(obj, size, pos)
		...
		obj.Image = "rbxassetid://96720490324068"
		pcall(function()
			obj.ImageContent = Content.fromAssetId(96720490324068)
		end)
	...
	if hub then
		...
		hub.Position = UDim2.new(0, 545, 0, 48)
		hub.Size = UDim2.new(0, 95, 0, 184)
		hub.ZIndex = 7
		corner(hub, 7)
		stroke(hub, C.stroke, 1, 0.2)
	end
...
local function C_93()
	local script = G2L["93"];
	local yes =  script.Parent
	-- Do not make the whole Main draggable: on mobile that can steal the tap
	-- that should focus the code TextBox and prevent the on-screen keyboard.
