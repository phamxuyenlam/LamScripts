...
local function C_71()
    ...
    -- Preview loader: use the direct Roblox asset ID.  ShiftLock is a
    -- horizontal 512x277 image, so it deliberately uses no rbxthumb fallback.
    local function loadPreview(frame, assetId)
        ...
        -- Let Roblox load exactly the asset assigned above.  There is no
        -- thumbnail/HTTP fallback here, so the ID cannot be replaced by a
        -- different image format later.
        task.defer(function()
...
do
	...
	if hubModal then
		...
		if shiftLockImage and (shiftLockImage:IsA("ImageLabel") or shiftLockImage:IsA("ImageButton")) then
			...
			shiftLockImage.BackgroundTransparency = 1
			pcall(function()
...
-- ============================================================
-- ============================================================
-- ReSave final UI repair pass.
-- One deterministic pass for main corners, Script Hub and Options.
-- ============================================================
do
    local root = G2L["1"]
    local main = G2L["6"]
    if root and main then
        ...
        local function stroke(obj, color, thickness, transparency)
            if not obj then return end
            local st = obj:FindFirstChild("ReSaveFinalStroke")
            if not st then
                st = Instance.new("UIStroke")
                st.Name = "ReSaveFinalStroke"
                st.Parent = obj
            end
            st.Color = color
        ...
        local function setTextButtonStyle(btn, size, pos)
            ...
            btn.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
            btn.TextColor3 = Color3.fromRGB(244, 241, 252)
            btn.AutoButtonColor = false
            corner(btn, 6)
        end

        -- Main window: clip the actual surfaces as well as the root so
        -- rounded corners cannot be overpainted by square child frames.
        main.ClipsDescendants = true
        main.BorderSizePixel = 0
        main.BackgroundTransparency = 0
        main.BackgroundColor3 = Color3.fromRGB(8, 7, 11)
        corner(main, 10)
        stroke(main, Color3.fromRGB(151, 94, 246), 1.5, 0.22)
        ...
        local toolbar = main:FindFirstChild("ReSaveToolbar")
        if toolbar then
            toolbar.Position = UDim2.new(0, 1, 0, 243)
            toolbar.Size = UDim2.new(1, -2, 0, 39)
            toolbar.BorderSizePixel = 0
            toolbar.ZIndex = 18
        ...
        local actionPositions = {
            Execute = 7, Clear = 96, OpenFile = 185, SaveFile = 274,
            Options = 363, Attach = 452, ScriptHub = 541,
        }
        for name, x in pairs(actionPositions) do
            local b = main:FindFirstChild(name)
            if b then
                b.ZIndex = 60
                b.Position = UDim2.new(0, x, 0, 247)
                b.Size = UDim2.new(0, 82, 0, 30)
                b.BorderSizePixel = 0
                corner(b, 7)
        ...
        -- Hide the legacy right-side quick-file container.  The Open File
        -- popup remains the user-facing file picker.
        local legacyHub = main:FindFirstChild("MainFunc") and main.MainFunc:FindFirstChild("ScriptHub")
        if legacyHub then
            legacyHub.Visible = false
        end
        ...
        if hub then
            ...
            hub.ZIndex = 100
            corner(hub, 10)
            stroke(hub, Color3.fromRGB(151, 94, 246), 1.2, 0.18)

            local hubBg = hub:FindFirstChild("Background")
            if hubBg then
                hubBg.Position = UDim2.new(0, 1, 0, 1)
                hubBg.Size = UDim2.new(1, -2, 1, -2)
                hubBg.BorderSizePixel = 0
                hubBg.BackgroundTransparency = 0
                hubBg.BackgroundColor3 = Color3.fromRGB(18, 16, 23)
                hubBg.ImageTransparency = 1
                hubBg.ZIndex = 100
                hubBg.ClipsDescendants = true
                corner(hubBg, 9)
            end

            local list = hubBg and hubBg:FindFirstChild("ScrollingFrame")
            if list then
                ...
                local pad = list:FindFirstChild("ReSaveFinalListPadding")
                if not pad then
                    pad = Instance.new("UIPadding")
                    pad.Name = "ReSaveFinalListPadding"
                    pad.Parent = list
                end
                ...
                local names = {"Dex", "RemoteSpy", "UnnamedESP", "ScriptDumper", "ShiftLock"}
                for i, name in ipairs(names) do
                    ...
                    if btn and btn:IsA("TextButton") then
                        ...
                        local bp = btn:FindFirstChild("ReSaveFinalButtonPadding")
                        if not bp then
                            bp = Instance.new("UIPadding")
                            bp.Name = "ReSaveFinalButtonPadding"
                            bp.Parent = btn
                        end
            ...
            -- Header: keep icon and title separate so they never touch.
            ...
            if title then
                ...
                title.Active = false
                title.ZIndex = 138
                title.Position = UDim2.new(0, 40, 0, 1)
                title.Size = UDim2.new(1, -98, 0, 28)
                title.BackgroundTransparency = 1
                title.TextColor3 = Color3.fromRGB(244, 241, 252)
                title.TextXAlignment = Enum.TextXAlignment.Left
                title.TextYAlignment = Enum.TextYAlignment.Center
                title.Text = "ReSave - Script Hub"
            end
            ...
            if minus then
                ...
                minus.ZIndex = 200
                minus.Position = UDim2.new(1, -38, 0, 4)
            ...
            local dragBar = hub:FindFirstChild("ReSaveHubFinalDragBar")
            if not dragBar then
                dragBar = Instance.new("Frame")
                dragBar.Name = "ReSaveHubFinalDragBar"
                dragBar.Parent = hub
            end
            dragBar.Position = UDim2.new(0, 38, 0, 0)
            dragBar.Size = UDim2.new(1, -82, 0, 30)
            dragBar.BackgroundTransparency = 1
            dragBar.BorderSizePixel = 0
            dragBar.Active = true
            dragBar.ZIndex = 145
            dragBar.Draggable = false

            local desc = hub:FindFirstChild("Description")
            if desc then
                desc.Visible = true
                desc.ZIndex = 160
                desc.Position = UDim2.new(0, 125, 0, 205)
                desc.Size = UDim2.new(0, 275, 0, 39)
                ...
                corner(desc, 6)
                local dp = desc:FindFirstChild("ReSaveFinalDescPadding")
                if not dp then
                    dp = Instance.new("UIPadding")
                    dp.Name = "ReSaveFinalDescPadding"
                    dp.Parent = desc
                end
            ...
            if execute then
                setTextButtonStyle(execute, UDim2.new(0, 121, 0, 28), UDim2.new(0, 125, 1, -37))
                execute.ZIndex = 190
            ...
            if close then
                setTextButtonStyle(close, UDim2.new(0, 121, 0, 28), UDim2.new(0, 257, 1, -37))
                close.ZIndex = 190
                close.Text = "Close"
            end

            -- Preview containers use one stable horizontal rectangle.
            ...
            for _, name in ipairs(previewNames) do
                ...
                if frame and frame:IsA("Frame") then
                    ...
                    frame.Size = (name == "ShiftLock") and UDim2.new(0, 275, 0, 149) or UDim2.new(0, 275, 0, 126)
                    frame.BackgroundTransparency = 1
                    frame.BorderSizePixel = 0
                    frame.ZIndex = 150

                    local img = frame:FindFirstChild("Image")
                    if img and (img:IsA("ImageLabel") or img:IsA("ImageButton")) then
                        img.Visible = false
            ...
            -- ShiftLock is the only preview whose asset size is explicitly
            -- known: 512x277.  Use only the direct asset URI.
            ...
            if shiftFrame and shiftImage and (shiftImage:IsA("ImageLabel") or shiftImage:IsA("ImageButton")) then
                ...
                shiftImage.Image = "rbxassetid://94251318161622"
                ...
                task.defer(function()
                    local cp = game:GetService("ContentProvider")
                    pcall(function() cp:PreloadAsync({shiftImage}) end)
                    if shiftImage.IsLoaded == false then
                        pcall(function() shiftImage.Image = "rbxassetid://94251318161622" end)
                        pcall(function() cp:PreloadAsync({shiftImage}) end)
                    end
                end)
            end

            -- Move the entire popup, not just its title text.
        ...
        if options then
            ...
            options.ZIndex = 200
            corner(options, 10)
            stroke(options, Color3.fromRGB(151, 94, 246), 1.2, 0.18)

            local optionBody = options:FindFirstChild("MainFrame")
            if optionBody then
                optionBody.Position = UDim2.new(0, 1, 0, 31)
                optionBody.Size = UDim2.new(1, -2, 1, -32)
                optionBody.BorderSizePixel = 0
                optionBody.BackgroundTransparency = 0
                optionBody.BackgroundColor3 = Color3.fromRGB(18, 16, 23)
                optionBody.ZIndex = 201
                optionBody.ClipsDescendants = true
                corner(optionBody, 9)
            end

            local optionIcon = options:FindFirstChild("Icon")
            if optionIcon then
                optionIcon.Position = UDim2.new(0, 9, 0, 3)
                ...
                optionIcon.ZIndex = 220
                optionIcon.Image = "rbxassetid://96720490324068"
            end

            local optionTitle = options:FindFirstChild("Title")
            if optionTitle then
                optionTitle.Position = UDim2.new(0, 41, 0, 2)
                optionTitle.Size = UDim2.new(1, -82, 0, 28)
            ...
            if optionClose then
                ...
                optionClose.Position = UDim2.new(1, -36, 0, 4)
                optionClose.Size = UDim2.new(0, 27, 0, 23)
                optionClose.ZIndex = 230
                optionClose.BackgroundColor3 = Color3.fromRGB(28, 22, 38)
                optionClose.BackgroundTransparency = 0
                optionClose.BorderSizePixel = 0
                optionClose.Text = ""
                corner(optionClose, 5)
            ...
            local optionLabels = {
                FPSUnlocker = {text = "FPS Unlock [BETA]", y = 49},
                AutoExec = {text = "Auto Exec scripts", y = 83},
                InternalUI = {text = "Internal UI", y = 117},
                TopMost = {text = "TopMost", y = 151},
            }
            for name, info in pairs(optionLabels) do
                local label = options:FindFirstChild(name)
                if label then
                    label.Visible = true
                    label.Position = UDim2.new(0, 18, 0, info.y)
            ...
            local toggleY = {
                ToggleFPSUnlocker = 49,
                ToggleAutoExec = 83,
                ToggleInternalUI = 117,
                ToggleTopMost = 151,
            }
            for name, y in pairs(toggleY) do
                local toggle = options:FindFirstChild(name)
                if toggle then
                    toggle.Visible = true
                    toggle.Active = true
                    toggle.Draggable = false
                    toggle.Position = UDim2.new(1, -42, 0, y + 2)
                    toggle.Size = UDim2.new(0, 20, 0, 20)
                    toggle.BorderSizePixel = 0
                    toggle.ZIndex = 220
                    toggle.BackgroundColor3 = (toggle.Text == "x") and Color3.fromRGB(124, 78, 201) or Color3.fromRGB(40, 34, 50)
            ...
            local optionButtons = options:FindFirstChild("Buttons")
            if optionButtons then
                optionButtons.Position = UDim2.new(0, 18, 0, 194)
                optionButtons.Size = UDim2.new(1, -36, 0, 55)
                optionButtons.BackgroundTransparency = 1
                optionButtons.BorderSizePixel = 0
                optionButtons.ZIndex = 210

                local buttonLayout = optionButtons:FindFirstChildOfClass("UIListLayout")
                if buttonLayout then
                    buttonLayout.Enabled = false
                end

                local rejoin = optionButtons:FindFirstChild("Rejoin")
                if rejoin then
                    rejoin.Visible = true
                ...
                local discord = optionButtons:FindFirstChild("Discord")
                if discord then
        ...
        -- Keep the toolbar Options button responsible for opening the same
        -- centered popup.  The original handler stays intact.
        local optionsButton = main:FindFirstChild("Options")
        if optionsButton and options and not optionsButton:GetAttribute("ReSaveFinalOptionsOpenBound") then
            optionsButton:SetAttribute("ReSaveFinalOptionsOpenBound", true)
