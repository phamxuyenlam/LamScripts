-- ReSave Remake made by @chillz_azy (yt)
...
local function C_51()
	...
	script.Parent.savescript.MouseButton1Up:Connect(function()
		...
		makefolder("ReSaveSavedScripts")
		writefile("ReSaveSavedScripts/"..script.Parent.scriptname.Text..".lua", module:GetActiveTextbox().Frame.Textbox.Text)
	end)
...
local function C_84()
	...
	for index, value in pairs(listfiles("ReSaveSavedScripts")) do
		...
		if isfile(value) then
			task.wait(0.05)
            local editedString = string.gsub(value, [[ReSaveSavedScripts\]], "")
...
local function C_92()
	...
	script.Parent.savescript.MouseButton1Up:Connect(function()
		...
		if isfile(script.Parent.scriptname.Text) then
			...
			script.Parent.Title.Text = "ReSave - Open File (File not found!)"
			task.wait(1)
			script.Parent.Title.Text = "ReSave - Open File"
		end
	end)
...
task.spawn(C_92);
-- ============================================================
-- Modern Black + Purple UI Theme
-- Visual-only customization: keeps the existing simulator logic.
-- ============================================================
do
	local root = G2L["1"]
	local main = G2L["6"]
	...
	local colors = {
		...
		surface2 = Color3.fromRGB(24, 20, 31),
		editor = Color3.fromRGB(12, 12, 16),
		text = Color3.fromRGB(245, 243, 255),
		muted = Color3.fromRGB(169, 162, 188),
		purple = Color3.fromRGB(124, 78, 201),
		purpleBright = Color3.fromRGB(157, 103, 255),
		purpleDeep = Color3.fromRGB(74, 42, 120),
		stroke = Color3.fromRGB(62, 42, 83),
		strokeBright = Color3.fromRGB(139, 92, 246),
	}
	...
	local mainStroke = main:FindFirstChild("ModernStroke")
	if not mainStroke then
		mainStroke = Instance.new("UIStroke")
		mainStroke.Name = "ModernStroke"
		mainStroke.Parent = main
	end
	mainStroke.Color = colors.strokeBright
	mainStroke.Thickness = 1
	mainStroke.Transparency = 0.35

	local background = main:FindFirstChild("Background")
	if background then
		background.BackgroundColor3 = colors.bg
	...
	if panel then
		...
		local gradient = panel:FindFirstChild("ModernGradient")
		...
		gradient.Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, Color3.fromRGB(31, 23, 42)),
	...
	if title then
		...
		title.Size = UDim2.new(0, 520, 0, 27)
	end

	-- Make the three window controls compact and consistent.
	for _, name in ipairs({"Minimize", "Maximize", "Close"}) do
		local button = main:FindFirstChild(name)
		if button then
			button.BackgroundColor3 = Color3.fromRGB(25, 22, 31)
			...
			local stroke = button:FindFirstChild("ModernStroke")
			if not stroke then
			...
			local text = button:FindFirstChild("NameText")
			if text then
	...
	-- Main action row: evenly spaced, rounded, and readable.
	...
	for _, name in ipairs(actionNames) do
		local button = main:FindFirstChild(name)
		if button then
			button.Size = UDim2.new(0, 84, 0, 30)
			button.Position = UDim2.new(0, actionPositions[name], 0, 247)
			...
			local gradient = button:FindFirstChild("ModernGradient")
			if not gradient then
				gradient = Instance.new("UIGradient")
				gradient.Name = "ModernGradient"
			...
			gradient.Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0, Color3.fromRGB(34, 25, 46)),
				ColorSequenceKeypoint.new(1, Color3.fromRGB(21, 19, 26)),
			})
	...
	-- Editor
	local textboxes = main:FindFirstChild("MainFunc")
	if textboxes then
		local needs = textboxes:FindFirstChild("Needs")
		if needs then
			local editor = needs:FindFirstChild("Textbox")
			if editor then
				editor.BackgroundColor3 = colors.editor
				editor.BorderSizePixel = 0

				local frame = editor:FindFirstChild("Frame")
				if frame then
					frame.BackgroundColor3 = colors.editor

					local linebar = frame:FindFirstChild("Linebar")
					if linebar then
						linebar.BackgroundColor3 = Color3.fromRGB(17, 14, 23)
					end

					local code = frame:FindFirstChild("Textbox")
					if code then
						code.BackgroundColor3 = colors.editor
						code.TextColor3 = colors.text
					end

					local highlighted = frame:FindFirstChild("Highlighted")
					if highlighted then
						highlighted.BackgroundColor3 = Color3.fromRGB(67, 42, 91)
						highlighted.BackgroundTransparency = 0.75
					end
				end
			end
		end

		local hub = textboxes:FindFirstChild("ScriptHub")
		if hub then
			hub.BackgroundColor3 = Color3.fromRGB(14, 13, 18)
			local hubStroke = hub:FindFirstChild("ModernStroke")
			if not hubStroke then
				hubStroke = Instance.new("UIStroke")
				hubStroke.Name = "ModernStroke"
	...
	if floating then
		...
		floating.BackgroundColor3 = Color3.fromRGB(13, 11, 17)
		floating.BorderSizePixel = 0

		local corner = floating:FindFirstChildOfClass("UICorner")
		if corner then
			corner.CornerRadius = UDim.new(1, 0)
		end

		local gradient = floating:FindFirstChildOfClass("UIGradient")
		if gradient then
			gradient.Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0, Color3.fromRGB(49, 29, 67)),
				ColorSequenceKeypoint.new(1, Color3.fromRGB(11, 10, 14)),
			})
		end

		local stroke = floating:FindFirstChild("ModernStroke")
		if not stroke then
			stroke = Instance.new("UIStroke")
			stroke.Name = "ModernStroke"
			stroke.Parent = floating
		end
		stroke.Color = colors.strokeBright
	...
	-- Use the new logo in the visible windows too.
	for _, descendant in ipairs(root:GetDescendants()) do
		if descendant:IsA("ImageLabel") and descendant.Name == "Icon" then
			descendant.Image = "rbxassetid://96720490324068"
		end
	end

	-- Consistent modal surfaces and action buttons.
	local modalNames = {"SaveScript", "OpenScript", "OptionMenu", "ScriptHubMenu", "ScriptLog"}
	for _, name in ipairs(modalNames) do
		local modal = root:FindFirstChild(name)
		if modal then
			modal.BackgroundColor3 = colors.surface
			modal.BorderSizePixel = 0

			local corner = modal:FindFirstChildOfClass("UICorner")
			if not corner then
				corner = Instance.new("UICorner")
				corner.Parent = modal
			end
			...
			local stroke = modal:FindFirstChild("ModernStroke")
			if not stroke then
				stroke = Instance.new("UIStroke")
				stroke.Name = "ModernStroke"
				stroke.Parent = modal
	...
	if options then
		...
		if optionTitle then
			optionTitle.TextColor3 = colors.text
			optionTitle.FontFace = Font.new(
				"rbxasset://fonts/families/SourceSansPro.json",
				Enum.FontWeight.SemiBold,
				Enum.FontStyle.Normal
			)
		...
		for _, child in ipairs(options:GetDescendants()) do
			if child:IsA("TextLabel") then
				child.TextColor3 = colors.text
			elseif child:IsA("TextBox") then
				child.BackgroundColor3 = colors.editor
				child.TextColor3 = colors.text
			end
		end

		for _, name in ipairs({"ToggleAutoExec", "ToggleFPSUnlocker", "ToggleInternalUI", "ToggleTopMost"}) do
			local toggle = options:FindFirstChild(name)
			if toggle then
				local corner = toggle:FindFirstChildOfClass("UICorner")
				if not corner then
					corner = Instance.new("UICorner")
				...
				local stroke = toggle:FindFirstChild("ModernStroke")
				if not stroke then
					stroke = Instance.new("UIStroke")
					stroke.Name = "ModernStroke"
		...
		if optionButtons then
			for _, child in ipairs(optionButtons:GetChildren()) do
				if child:IsA("TextButton") then
					child.BackgroundColor3 = colors.surface2
					child.TextColor3 = colors.text
