...
local function C_71()
    ...
    local function loadPreview(frame, assetId)
        ...
        local candidates = {
            "http://www.roblox.com/asset/?id=" .. tostring(assetId),
...
do
	...
	if root and main then
		...
		if hub then
			...
			if shiftImage and (shiftImage:IsA("ImageLabel") or shiftImage:IsA("ImageButton")) then
				local asset = "rbxassetid://94251318161622"
				local thumb = "rbxthumb://type=Asset&id=94251318161622&w=420&h=420"
				shiftImage.Visible = true
				shiftImage.Image = asset
				...
				task.delay(0.75, function()
					if shiftImage and shiftImage.Parent and shiftImage.IsLoaded == false then
						shiftImage.Image = thumb
					end
				end)
			end
...
do
    ...
    if hub then
        ...
        -- Force the new ShiftLock preview method immediately.
        local shiftFrame = hub:FindFirstChild("ShiftLock")
        local shiftImage = shiftFrame and shiftFrame:FindFirstChild("Image")
        if shiftImage and (shiftImage:IsA("ImageLabel") or shiftImage:IsA("ImageButton")) then
            shiftImage.Visible = true
            shiftImage.Image = "http://www.roblox.com/asset/?id=94251318161622"
            shiftImage.ImageTransparency = 0
            shiftImage.ImageColor3 = Color3.fromRGB(255, 255, 255)
            shiftImage.ScaleType = Enum.ScaleType.Fit
            task.spawn(function()
                local candidates = {
                    "http://www.roblox.com/asset/?id=94251318161622",
                    "rbxassetid://94251318161622",
                    "rbxthumb://type=Asset&id=94251318161622&w=420&h=420",
                }
                for _, candidate in ipairs(candidates) do
                    shiftImage.Image = candidate
                    pcall(function()
                        game:GetService("ContentProvider"):PreloadAsync({shiftImage})
                    end)
                    task.wait(0.35)
                    if shiftImage.IsLoaded then break end
                end
            end)
        end
